...
function receive(st, id, cfg = RONDO) {
  ...
  if (p.team === st.possession.team) {
    ...
    if (pick) {
      ...
      if (st.full && cfg.corpsArret) corpsArret(st, p, cfg.corpsArret);   /* (326) le corps s'arrête avec son contrôle (corps-arret.js) */ p._controleAt = st.t; st._settling = { ev: st.events.length, id, at: st.t + T, ...(st._pousse ? { pousse: true } : {}) };   /* (286) la date du contrôle : la touche qui engage la lit */
      st.events.push({
...
export function rondoStep(st, dt, cfg = RONDO) {
  ...
  if (st.phase === 'carry') {
    ...
    if (!st._drb) st._drb = makeDribbler(st.full && cfg.prise !== false ? { prise: cfg.prise ?? 0.62, ...(cfg.allongeTouche ? { reach: cfg.allongeTouche } : {}) } : {});   /* (305) l'allonge de la touche de conduite à la portée du pied (le rendu la montre) ; absente : 1,15 d'hier */ // la touche au pied (lot 58) — doc : match-config
    ...
    // le vrai joueur POSE le pied sur un ballon lent à portée). Le cône et le prenable tiennent.
      ...
      && dansCone(c.yaw, c.p[0], c.p[2], st.ball.p[0], st.ball.p[2], cfg.ramasse.cone ?? 80))) && !contested && d2(c.p, st.ball.p) < cfg.captureRadius && (!st.full || cfg.prisePied === false || balPrenable(st.ball, c.p[0], c.p[2], cfg.prisePied ?? 0.5, 0.5, st.full && cfg.priseRelative && intentFresh ? c.v : null /* (300) la passe adoptée reprend le ballon qui roule avec son homme : la fuite en relatif (dribble.balPrenable) */)) && (!st.full || cfg.priseCone === false || dansCone(c.yaw, c.p[0], c.p[2], st.ball.p[0], st.ball.p[2], cfg.priseCone ?? 100))) {
    ...
    const PL = st.full && cfg.presseLue ? presseLueDe(st, c, cfg.presseLue, cfg, tacDe(st, c.team).tempo) : null; c._presse = PL ? { t: st.t, ...PL } : null; const calm = (PL ? !PL.presse : foeBody > cfg.calmFoe) && !relaisChaud;   /* (289) LA PRESSION SE LIT AU TEMPS D'ARRIVÉE (presse-lue.js) : le calme n'est plus « personne à 1,8 m » mais « personne n'arrive » */
    ...
    if (st.hold >= Math.max(0, cfg.holdMin - cfg.windupBudget) && (porte || (decide && st.full && cfg.cadence)) && (!settleGate || contested)) {
      ...
      if (contested) {
        ...
        const dosR = st.full && cfg.retournement && !engagementCall && choice && !choice.cross && choice.style !== 'lofted' && !st.restart && st.phase === 'carry' && !pressCall && !jeteCall && !(c._presse?.t === st.t && c._presse.presse) && st.players[choice.to.id]
    ...
    if (released) {
      ...
      for (const p of st.players) {
        ...
        const d = d2(p.p, st.ball.p);
        if (d < cfg.receiveRadius && st.ball.p[1] < (st._enchaine && st.t < st._enchaine.until ? (cfg.enchainement?.prise ?? 0.45) : 1.9) && d < bestD && (!(st.full && cfg.priseAuPied) || d < (cfg.priseAuPied.r ?? 0.5) || st.ball.p[1] > 0.5 || ((st.ball.v[0] - p.v[0]) * (st.ball.p[0] - p.p[0]) + (st.ball.v[2] - p.v[1]) * (st.ball.p[2] - p.p[2])) / Math.max(1e-3, d) > -(cfg.priseAuPied.approche ?? 0.3)) /* (333) LA PRISE AU PIED : mesuré, le contrôle partait à l'ENTRÉE du rayon (ballon-corps à la prise p50 0,82 / p90 0,85 m — toujours au bout de la jambe, le rendu étirait la jambe ou glissait le corps). Le ballon qui VIENT encore au joueur se laisse venir jusqu'au pied (r m) ; qui ne vient plus (le plus près de sa course) ou aérien : la prise d'hier. Absente : hier */ && !(st.full && cfg.enchainement && p.team === st.lastTouch && st.ball.p[1] >= (cfg.poitrine?.min ?? 1.15) && st.pitch.inBox(p.p[0], p.p[2], Math.sign(st.pitch.attackGoal(p.team).x || 1)))) { bestD = d; taker = p.id; }   /* (note 386) un ballon à hauteur de poitrine dans la surface adverse est à la POITRINE (chestStep), pas au pied : la prise le volait avant l'enchaînement (mesuré en page : 'amorti-poitrine' possédé) */
      }
