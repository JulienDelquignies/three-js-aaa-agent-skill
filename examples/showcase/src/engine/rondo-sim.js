...
import { makeDribbler, dribbleStep, dribbleSteer, touchDistance, balPrenable, dansCone, toucheCadenceT } from './dribble.js'; import { RONDO, assignJobs, choosePass, strikingFoot, rondoInternals, enLance } from './rondo.js'; import { attendDe } from './ouverture.js';
...
export function rondoStep(st, dt, cfg = RONDO) {
  ...
  if (st.phase === 'carry') {
    ...
    if (!st._drb) st._drb = makeDribbler(st.full && cfg.prise !== false ? { prise: cfg.prise ?? 0.62, ...(cfg.pasPortee ? { pasPortee: cfg.pasPortee } : {}), ...(cfg.surface ? { sol: cfg.surface } : {}), ...(cfg.conduite?.cadence ? { rdvPied: true } : {}) } : {}); // la touche au pied (lot 58) — doc : match-config
    ...
    const SR = st.full && cfg.conduiteSerree ? serrePorteurDe(st, c, cfg.conduiteSerree, cfg) : null;   /* (290) LA CONDUITE SE SERRE SOUS PRESSION (serre.js) */ const pl = { p: [c.p[0], c.p[2]], speed: c.speed, vPlan: st.full && cfg.conduite?.couple ? c._vVoulue : undefined, toucheT: st.full && cfg.conduite?.cadence ? toucheCadenceT(cfg.conduite.cadence, c.speed) : undefined, cage: st.full && cfg.murCorps && st.area ? [st.area[0] / 2, st.area[1] / 2, cfg.murCorps] : undefined, heading, want, turnRate: 0, leadF: (c.skill?.dribbleLeadF ?? 1) * (st.full && cfg.conduite?.lead ? cfg.conduite.lead : 1), ...(SR ? { serreK: SR.k, serreV: SR.dv } : {}), ...(st.full && cfg.toucheCorps ? { corps: heading, corpsK: cfg.toucheCorps } : {}),
