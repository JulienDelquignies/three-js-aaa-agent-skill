...
import { BallBody } from './ball-body.js'; import { porteePasseDe } from './portee.js';
...
export function choosePass(st, cfg = RONDO) {
  ...
  for (const m of mates(st, c.team)) {
    ...
    if (d < cfg.passRange[0] || d > (st.full && cfg.porteePasse ? porteePasseDe(rMax, m, c, cfg.porteePasse) : rMax)) continue;   // (296) LA PORTÉE EST CELLE D'UN PIED (portee.js)
    ...
    if (style === 'lofted' && st.full && cfg.porteePasse?.retrait?.tous && st.pitch && d >= (cfg.porteePasse.leve ?? 32) && (m.keeper || Math.abs(st.pitch.ownGoal(c.team).x - lead[0]) < st.pitch.dims.box.depth + (cfg.porteePasse.retrait.marge ?? 6))) style = 'driven';   // (313b, retrait.tous) …PAR TOUS LES CHEMINS : la diagonale (bascule) et la ligne bouchée levaient aussi la passe longue vers son but (1 / 4 × 900 s au bloc 313)
