...
export const MATCH = {
  ...
  contreZones: { win: 10, lance: 4, tenue: 1, sprintDe: 8, prof: 20, avance: 6, annexe: 14, cadence: 0.6 },   // LES TROIS ZONES D'ENTRÉE DE SURFACE EN CONTRE (242, st.full — phases.js, précepte 4.2 Elsner) : en contre (regain < win s — l'arrivée demande 6-10 s) et ballon QUI FILE (ballon ou porteur à ≥ lance m/s vers l'avant, le porteur ne tient pas plus de tenue s — sinon trois sprints s'allumaient après chaque regain : la fourmilière), les trois attaquants les plus avancés (hors porteur, hors course) prennent centre / annexe côté ballon / annexe lointaine (le plus fort rôle appel), cibles FIXES à l'entrée de la surface (prof m de la ligne de but, ou ballon + avance m si le ballon est déjà plus avancé) sous la Loi 11, z 0 / ± annexe m (l'excentré en position intermédiaire, jamais deuxième latéral) ; amplitude × axe transition (0 : rien, 0,5 : plein) ; les élus sprintent (burst contre-zone) tant qu'ils sont à plus de sprintDe m de leur zone. Absente : les pointes courent où l'appel les mène (trois zones occupées 3,7 % des contres)
  ...
  loi12: { avantage: 1.8, contact: 0.9, mur: 9.15, jaune: 2, murTrot: 1.6 }, viragesLisses: { taux: 6, tau: 0.15, des: 2.0, frein: 0.2, arrivee: 1.5, demiTour: 100 /* (302) le demi-tour est un freinage — movement.js ; absent : hier */ }, sePoser: { v: 1.0, fen: 1.0, cos: 0 } /* (303) LE RECEVEUR FACE AU BALLON SE POSE (match-sim, movement) — la dernière seconde du vol, s'il ne court pas avec le ballon, sa pointe à v m/s ; null : hier */, aligneRecev: { marge: 0.3, reach: 0.35, cadence: 0.1, h: 0.9, aval: 0.15 } /* (309) LE RECEVEUR S'ALIGNE AVANT LA PRISE — doc foulee.js cibleAlignee ; absent : hier */, remiseDevant: true /* (303) la remise va où sera le receveur (premiere-intention, le rendez-vous du 281) ; null : hier */, toucheAxe: { plancher: 1.6 } /* (302) LA TOUCHE SE DOSE SUR L'ALLURE DANS SA DIRECTION (dribble.js) : « aucun contrôle n'est réussi » (retour du 25/09, Vercel) — 22 % des contrôles propres rendaient le ballon libre 0,5 s après, le porteur retiré à 2,2 m sans événement ; la touche de retournement partait à la vitesse scalaire du corps (4-5 m/s, ~8 m de roule), le corps tournait en arc 1,1 s à 140° de sa cible ; null : hier au bit */, plantVitesse: true, sortieAerienne: { bas: 1.6, haut: 2.3, zone: 8, large: 3, vitesse: 5.5, reaction: 0.2, marge: 0.1, detente: 1.0, duel: 7, saut: 0.5, portee: 40, poing: 0.4, poingV: 12 },   // (B4) murTrot : les deux hommes du mur sont les deux qui y seront les PREMIERS (les plus près du mur) et ils y TROTTENT (× marche), null : les deux plus profonds au pas — (B5) viragesLisses : le cap demandé filtré (tau s) et borné (taux/v rad/s) dès des m/s, les courbes à la place du bang-bang latéral, null : les cassures d'hier — (B6) plantVitesse : le corps planté sous un acte a une vitesse NULLE (sauf élan / tête armée / mains ; hier p.v gardait sa dernière valeur : 84 % des images plantées à > 1 m/s, l'armé de passe à 4,4 m/s p50), null : hier
  carton: { base: { accrochage: 0.35, 'tacle-debout': 0.3, 'charge-derrière': 0.6, 'tacle-glissé': 0.5, 'tacle-glissé-derrière': 0.95 }, defaut: 0.3, vitesse: 0.03, aggr: 0.3, arrache: -0.15, prometteur: 0.5, promCos: 0.5, promV: 3, promDist: 45, promCouvrants: 3, promLargeur: 8, dogso: 1.5, dogsoCos: 0.6, dogsoDist: 30, couvrants: 0, largeur: 18, jaune: 0.8, s: 0.15, tally: 2.0, rouge: 1.4, retenue: 0.55, reticence: 0.25 },   // LE CARTON JUGE LA NATURE (257, referee.adjugeFaute — la carte du book, Modèle 12 §3, Bible 15 §5 : réel ~4 jaunes, 0,1-0,36 rouge, 5,4-6,8 fautes par jaune ; hier la récidive à 2 : 9 jaunes, 2 rouges). Chaque faute porte un score S = base[espèce] + vitesse × v(victime) + aggr × (aggrF − 1) + arrache (l'arraché pèse moins) + prometteur (la victime filait vers le but : cos > promCos, v ≥ promV, < promDist m, ≤ promCouvrants corps dans le couloir de promLargeur m devant sa course) ; DOGSO (cos > dogsoCos, ≤ couvrants couvrants dans la bande largeur, < dogsoDist m) : S = dogso → rouge direct (≥ rouge), sauf dans sa surface sur un tacle : jaune + penalty. P(jaune) = σ((S − jaune − réticence si déjà averti) / s) au flux seedé ; l'ardoise cumule S et vaut jaune à tally (fautes répétées). L'averti se retient : × retenue sur l'accrochage et le glissé imprudent (duel). null : la récidive à 2 (loi12.jaune) au bit
  ...
  choix: { centreP: 0.4 } /* (335) le choix du porteur en valeur attendue (choix.js) — { T0 0,005, lucidite 0,7, pressionT 0,8, perte 1, tir 1, garde0 0,62, garderP 0,35, conduiteMax 6, muette 0,004, centreP, centreXg 0,11, pref 1, epoque 1,2 } ; mesuré 16 × 90 min (avec ellipse σ0 4) : tirs 19,5 → 25,0, distance p50 12 → 14,7 m, centres 10,1 → 13,1, buts 3,9 → 3,45 ; null : l'argmax des notes d'hier */, gkPied: { touche: 0.35, presse: 0.7, avance: true /* (26/09) le spot de distribution jamais derrière le gardien ; absent : le spot fixe à 4,5 m d'hier */ },   // 179 : LE PIED DU GARDIEN — la touche COLLÉE
  ...
  uneToucheVive: { press: 3.4, base: 0.35 /* (297) LA REMISE AU LIVRE : 0,7 → 0,35 — la une-touche au calme relevée par le 216 (4,3 % alors) avait dépassé la cible : 44-60 par équipe et par match, courtes, lentes, en retrait (sonde-302 ; le book 15-25). Mesuré 4 × 90 min : 49,5 → 35, passes tenues ; à 0,2 les passes baissent (389 → 365). 0,7 : hier */, dMin: 2.5, court: 7, capCourt: 8.5, couloir: 0.9, chas: 0.22, mene: 0.5, capRelais: 10, relaisPrio: true },   // 218c : LE MUR REMET AU COUREUR — sa cible dans la course (mene), le cap du retour (capRelais : 7-9 m/s requis, 6 permis), et le relais chaud faisable PASSE DEVANT (relaisPrio : sans bloqueur la marge d'un appui vaut 99). Mesuré : murs en une touche 4 → 9/16, retours 32 → 50 % (réel ~50), une-touche 77 % tenue. (Les mêmes boutons, tentés au 218 avant que le lanceur sprinte, avaient été rejetés.)  // 216 : LA PREMIÈRE
  ...
  serreRouge: { rayon: 26, serre: 0.45 },  // 192 : LA ZONE ROUGE SE SERRE — le point d'appui à
                          // < 26 m du but défendu se marque AU CONTACT (garde ×0,45, l'homme
                          // prime la bande, le suivi continu). Absente : le marqueur à 3,8 m
                          // d'hier (66 % de retournements dos-au-but).
  gkAuDevant: { rayon: 25, mene: 0.4, soutien: 5, plafond: 16 },  // 190 : LE GARDIEN VIENT AU RETRAIT — le ballon de
  ...
  contreTir: { vMin: 13, h: 1.3, rayon: 0.38, bruit: 1.1 },
  contre: { porte: 8, portee: 2, pres: 1.0, loin: 2.5, beta0: 3, surface: 1.5, dernier: 0.8, aggr: 4, jambe: 0.6, lamJambe: 1.0, lamJete: 1.6, vLat: 2.2, tau: 0.18, corps: 0.28, max: 1.2, duree: 1.2, couloir: 0.2, ec: 0.55, issues: [0.66, 0.08, 0.18, 0.08] },   // LE CORPS QUI CONTRE (258b, duel.contreEngage / contreTir — la carte du book, Modèle 10 §5 : réel 27 % de tirs contrés, mesuré 3-7 %) : à l'ARMÉ du tir, chaque défenseur devant le tireur (≤ porte m, ≤ portee m de la ligne) s'engage avec P = σ(beta0 + surface × 1,5 dans la surface + dernier × 0,8 pour le dernier défenseur + aggr × (aggrF − 1)) — jambe tendue (lam 1,0) à ≤ jambe m de la ligne, jeté (lam 1,6) au-delà — et court sur la ligne ballon → point visé (job 'contre', burst) ; son rayon d'obstruction R = corps + lam × vLat × (t − tau)⁺ ≤ max remplace le rayon fixe du 176 ; le tireur tire dans le trafic (couloir m au lieu de shotClear) ; les issues du contact (renvoi e_c / amorti / sortie / déviation à 0,66 / 0,08 / 0,18 / 0,08 — Modèle 10 §5.2). l'engagé vise le point de la ligne à pres…loin m du ballon. Mesuré 8 × 45 min : contrés 24 % (réel 27 ; avant 4-7) — la porte à 8 m parce que les défenseurs FERMENT en courant pendant l'armé (0,47 s) ; beta0 3 : l'engagement est « à faible coût cognitif et à forte fréquence » (P 0,95), l'attribut module aux marges. null : le bloc de champ du 176 au bit   // 176 : LE BLOC DE CHAMP — le
  ...
  finition: { sigma0: 1.4, aniso: 2.0, pied: 0.29, press: 5, kappa: 1.35, fatigue: 0.2, vMax: 22, gamma: 1.2, dist: 0.012, sigmaV: 0.08, sousDose: 0.05, sousDoseP: 0.1, fatigueV: 0.06, hauteur: { bas: 0.35, mi: 1.0, lucarne: 1.95, p: [0.62, 0.30] } },   // L'ÉCHELLE DE FINITION (258, strike-sim — la carte du book) : le TIR dévie EN ANGLE à la frappe, σψ = sigma0° × finF (finishing, identité à 50) × (1 + pied × weakF du mauvais pied) × (1 + kappa × composureF/1,075 × P) × (1 + fatigue (1 − stam)) × (v/vMax)^gamma × (1 + dist (d − 12)⁺), σθ = aniso × σψ (on manque au-dessus deux fois plus qu'à côté), vitesse log-normale (sigmaV) SOUS-DOSÉE (sousDose + sousDoseP × P + fatigueV (1 − stam)). P = 1 au corps (≤ 1 m), 0 au plateau press m (la distance, en attendant le temps d'arrivée du Modèle 07) ; vMax = le plafond de frappe DU MOTEUR (répertoire 39 : 16,5-21,5 m/s — le (v/vMax)^γ du book vit relatif au tireur, pas aux 35 m/s du réel) ; hauteur : le point visé des FRAPPES DE BUT (placé, puissance, enroulée, tendu — le ras-de-terre, le pointu, la volée, le lob gardent la hauteur de leur geste) est un TIRAGE (bas 0,35 m 62 %, mi-hauteur 1,0 m 30 %, lucarne 1,95 m 8 % — Modèle 10 §3.4), l'élévation compense la chute : le tir au-dessus existe. Remplace le σ métrique du point visé et le souffle du 145 sur le tir. null : le 145 d'hier, au bit
  ...
  poitrine: { min: 1.15, max: 1.55, reach: 0.55, kill: 0.78 }, orientationPasse: { marge: 0.6, fenetre: 40, tourner: 60, presse: 2.2, engagement: true, ouvre: 0.3, vTour: 1.2, anticPresse: 0.4, talon: true, talonP: 0.03 /* (315) la talonnade est rare : un tirage par possession × gesteF — strike-sim.js ; absent : hier */ }, verticalite: { avance: 0.32, plafond: 20, profond: 8, zone: 14, rayon: 4, devant: 6, espace: 3, portee: 30, dos: 3, libre: 4, retrait: 3, dosPlein: 10, dosLong: 12, retraitLong: 6, appel: true }, decalage: { v: 2.0, fixe: 5, lat: 3, cote: 1.3, tenir: 0.7, ballon: 1.1, foe: 3.0, closing: 1.0, plancher: 0.3, pinRel: 0.4, chargeCourse: 2.0, sortie: { bite: 0.6 } }, toucheOrientee: { libre: 4, v: 1.5, dos: 0.3, devant: 4, champ: 3, sens: 1, elan: 0.6, lead: 0.4, leadMin: 0.25, leadMax: 0.7 /* (330) le contrôle serré — la référence FM (réception de Reijnders, 1/15 s : le ballon tué à 20-30 cm du pied) : 0,4 / 0,25-0,7 m (325 : 0,6 / 0,4-1,0 ; hier 1,0 / 0,6-1,6) */, tournant: 1, vMin: 1.0 /* (310) la touche en travers reste courte — touche-orientee.js ; absent : hier */, elanV: 2.5 /* (312) le lancé prend dans sa foulée */ }, elanConduite: { v: 3.5, espace: 8, k: 0.7, garde: 1.5 } /* (312) l'élan dans l'espace : la poussée suit la course, le plafond garde la vitesse de prise — elan-conduite.js, movement.js ; absent : hier */,   /* (399) LA TOUCHE ORIENTÉE : le receveur libre en course ou dos au jeu emmène le ballon du côté ouvert (touche-orientee.js) — null : la capture d'hier */   /* (397) LA CONDUITE QUI DÉCALE : l'épaule attaquée (match-sim), le crochet en course (skills-sim), le passement en course jusqu'à chargeCourse — null : l'hier */   /* (396) LA VERTICALITÉ : la passe qui avance rend le point doux, l'espace devant vaut, le retrait du porteur libre se paie (rondo.choosePass) — null : l'hier */   /* (395) LA PASSE DANS LE SENS DU GESTE : le retournement à l'adoption, la technique choisie pour le tour, l'engagement au holdMin (strike-sim.beginPass, rondo-sim) — null : l'hier */ enchainement: { but: 16, libre: 1.5, prise: 0.45, apres: 0.35, volee: { delai: 0.45, devant: 0.6, hauteur: 0.7, face: 1.0 }, retournee: { devant: 0.15, hauteur: 1.75 } }, /* (note 386) L'ENCHAÎNEMENT poitrine → volée (face au but dans la surface : le ballon posé devant, repris en delai s) / poitrine → retournée (dos au but, libre : remonté au-dessus de la tête, le ciseau armé au pas d'après) ; null = hier au bit */  // 182a : LA POITRINE — la fenêtre
  ...
  avantContact: { seuil: 0.9, rayon: 6, contact: 1.0, barre: 1.2, lecture: true },   // lecture (246) : seuil × anticipF — le bon anticipateur voit le presseur plus tôt ; (2 − anticipF) l'inversait (anticipation 90 : possession 47,6 c. 57,8 à 10 ; corrigé 50,6 c. 52,3)   // LA PASSE AVANT LE CONTACT (227, doc rondo-sim) : ETA du presseur ≤ seuil × (2 − anticipF) × composureF → tenue dispensée, barre abaissée
  ramasseur: { patience: 6, marge: 0.6 }, ramasseurs: { n: 4, marge: 2.4, vitesse: 3.6, portee: 0.7, patience: 12, colle: 2.5, rattrape: 14 }, boiterie: { duree: 25, ralenti: 0.3 }, entrant: { trot: 1.6 }, petitsGestes: { semelle: { tenue: 0.7, vMax: 0.8 }, mur: { delai: 0.6, duree: 1.6 }, teteDefensive: true, controleOriente: { angle: 45 }, feinteAppel: { cadence: 20, vMax: 2.2 }, applaudir: { n: 2, rayon: 22, vMax: 3.5, cadence: 10, cadenceJoueur: 20, decal: [0.15, 0.8], p: { arret: 0.7, tir: 0.35, duel: 0.4, glisse: 0.4, interception: 0.4, tacle: 0.4, recuperation: 0.15 } } }, conduiteNommee: { droit: 12, vite: 3.0, lent: 1.0 }, /* (note 388) LA CONDUITE NOMMÉE : chaque touche dit son pied et sa surface (intérieur / extérieur / cou-de-pied / semelle) sur la géométrie de sa poussée — droit ° de part et d'autre du regard = droit devant, cou-de-pied à ≥ vite m/s, semelle sous lent m/s ; null = la touche muette d'hier */ passements: { spot: 0.40, lat: 0.05, face: 100, foe: 3.5, fixe: 0.45, charge: 2.6, ballon: 0.75, envie: 2, plancher: 0.35 }, /* (note 385) LES PASSEMENTS DE JAMBES NOURRIS : le jockey jusqu'à foe m, le porteur posé qui FIXE un vis-à-vis au demi-front large (≤ face °) pendant fixe s, le ballon calé au point du clip (spot devant, lat de côté) dès l'entrée ; null = hier au bit */ /* (§ 10, note 384) LES PETITS GESTES : la semelle du preneur à la sortie de but (la remise attend tenue s), le gardien qui replace son mur (designer, regard tenu duree s), la tête armée du défenseur en dégagement, le contrôle orienté (scène : yawWant − yaw ≥ angle °), la feinte d'appel du soutien posé (une par cadence s) ; null = hier au bit */ /* (§ 8, note 383) LA BOITERIE : le fauché d'une faute grave boite duree s (gaitPose opts.boite via la scène, la pointe réduite de ralenti × k qui guérit) ; L'ENTRANT trotte (trot × marche) ; null = l'hier au bit */   /* (§ 5, ramasseurs.js — note 382) LES RAMASSEURS DE BALLE : n corps assis aux quarts des touches (marge m dehors) ; le plus proche trotte (vitesse) au ballon hors d'atteinte, le ramasse (à ≤ portee m), le roule au point (frottement mesuré), s'assoit ; la remise attend ; patience s : le point d'hier ; colle m : le roulé arrêté près du point s'y pose ; null = l'hier au bit */   // LE RAMASSEUR DE BALLE (225b, doc referee.ballFetch) : ballon hors du tablier + marge, ou quête > patience s → au point de remise
  referme: { part: 0.45, second: 0.225, recul: 1.5, reculSecond: 0.75, sortie: 2, zone: 40, note: 0 },   // note 0 (246) : la note de placement n'amplifie plus le recul ni le glissement — × posF coûtait au bon placeur 5,6 pts de possession (47,5 c. 53,1 sans note, 12 × 300 s) ; le placement est une précision (cfg.placement), pas une amplitude   // sortie/zone (245) : l'oblique ne recule que sur une VRAIE sortie (le sortant ≥ sortie m devant la ligne) et ballon à < zone m du but — absentes : le 237 au bit, qui reculait la ligne à chaque pression de milieu de terrain et effaçait les notes (gradation 504 → 94)   // part 0,5 → 0,45 (237) : l’axe marquage a retrouvé son identité (× 1 au milieu au lieu de × 0,9) — même glissement au bit qu’hier   // …et L'OBLIQUE 1+3 (237) : le voisin du sorti recule de recul m, le second de reculSecond, × posF × axe marquage — le V pointé vers le ballon ; recul absent : la ligne plate d'hier   // LA LIGNE SE REFERME (228, doc marquage.js) : un qui sort, les voisins glissent vers le trou × posF × axe marquage
  ...
  relance: { prof: -4, ecart: 1, pivot: 22, lateral: 30, pression: { presseurPosé: 6, presseurDirect: 14, dans20: 3, portee: 25 } },   // pression (223b) : le court/long du gardien se lit à la pression (doc keeper.js)   // brief : centraux y ±14-20 à x 8-14 m au renvoi, sentinelle 16-24 m, latéraux y ±28-33 à x 25-35
  ...
  salida: { zone: 30, portee: 25, pression: 3, prof: -1 },   // LA SALIDA LAVOLPIANA (239, salida.js) : ballon au central/gardien à < zone m, ≥ pression adversaires à < portee → le pivot entre les centraux (leur ligne + prof) ; absente : le pivot à 22 m d'hier
  conduc: { libre: 5, engage: 4, max: 12, pas: 6, tenue: 2 },   // LA CONDUCCIÓN (239, salida.js) : le central libre (> libre m) conduit droit devant jusqu'à max × axe style × visionF × arbitre.conduite, la tenue × tenue pendant l'appât ; absente : le cap d'hier
  gardeTiers: { proche: 2, milieu: 4, lireCible: 2.5 },   // lireCible : le presseur dont la cible est à plus de 2,5 m du porteur CADRE — il n'« arrive » pas pour la passe avant contact (227)
  marquageTenue: { gain: 0.8 },   // L'AFFECTATION SE TIENT (238, marquage.js) : le marqueur garde son homme sauf marqueur libre à moins de gain × son coût (0,6 ouvrait la surface : 53 tirs/90 — le marqueur hors peloton privait le bloc de sa couverture ; 0,8 : 38, marque 2,1 m) ; horsPeloton:false : le marqueur d'hier ne reste éligible qu'au peloton (marque 2,8 m) ; absente : le tirage à chaque image d'hier   // LA GARDE PAR TIERS (238, garde.js) : l'ombre de couverture respecte la garde 222 (elle posait 1,15 m avant le jockey — la garde n'atteignait jamais les corps), tiers proche 2 m ; absente : l'hier au bit
  ...
  porteAnticipe: { tau: 0.015, frein: 0.4, reprise: 0.8 },   // (retour utilisateur « le joueur oublie le ballon ») LE PORTÉ ANTICIPE : pendant l'armé, le ballon se porte au point de stance du corps APRÈS son glissement, au servo serré (tau) — hier il traînait 0,38 m derrière à 7,5 m/s, un armé sur cinq était refusé au contact (stance-au-contact), le ballon vendangé, le corps filait sans lui. Absente : l'hier au bit.
  /* LES DETTES DU LOT A12 (doc reference/58) — pausaPied : pendant la tenue du 253 le porteur va rechercher le ballon qui a filé (libre, < rayon m) avant de tenir ; recevoirSurPlace : passe dans les pieds (mène < marge m), au sol, non déviée, personne à < pression m → le receveur ATTEND au lieu du pas au-devant ; pasDeRecul : au départ du centre (fenetre s) le corps de boîte à fort appel marqué à < marque m recule de d m hors de l'angle mort ; chacune null = hier au bit */ pausaPied: { bloque: 1.6, rayon: 3.5, lent: 1.6 }, recevoirSurPlace: { marge: 0.6, pression: 4, vMin: 2.5 }, pasDeRecul: { d: 0.8, marque: 1.6, appel: 0.6, fenetre: 0.7 }, arbitreGestes: { siffler: 1.1, carton: 2.0, designer: 1.4, avantage: 1.4 } /* (A11 bis) LES GESTES DE L'ARBITRE : au sifflet le central s'arrête, porte le sifflet à la bouche puis désigne la direction du coup franc ; le carton se montre au fautif (bras levé, le corps tourné vers lui) ; l'avantage se signale en courant (les deux bras devant) — st.arbitre.geste { kind, at, until, dir, couleur }, une file (le sifflet passe devant) ; durées en s ; null = hier au bit (le central court sans gestes) */, fete: { elan: 1.4, glisse: 1.9, portee: 1.3, marche: 0.75, flairGlisse: 0.7, calme: 1.15, flairOreille: 0.45, poing: 1.05 } /* (A11) LE TEMPÉRAMENT DE LA JOIE : le buteur fête selon sa persona (flair ≥ flairGlisse : la glissade sur les genoux après elan s de course — down glisse s, le corps porté à portee × sa vitesse ; calm ≥ calme : le retour calme au pas (marche × la marche) ; flair ≥ flairOreille et posé : la main à l'oreille au coin ; burstiness ≥ poing : le poing ; sinon les bras levés) — l'événement celebration porte le geste, glissade est un événement à part ; null = hier au bit */, sol: { tenue: 0.9, corps: 0.9, aide: { rayon: 10, dist: 1.1, avant: 0.7, loin: 15, trot: 3.5 } } /* (A10 quater) sol.aide : le jeu arrêté (coup franc posé, ou ballon à > loin m), le coéquipier le plus proche (< rayon m, ni fauteur, ni porteur, ni preneur, ni gardien) vient se poster à dist m du fauché et lui tend la main (geste 'mainTendue') quand le relevé approche (down ≤ avant s) ; null = le relevé solitaire d'hier */ /* (A10 bis) LE SOL : le fauché reste à terre chute + tenue × (0,7 + 0,6 × v/6) s, lâche le ballon (phase loose) et n'agit plus couché ; personne ne marche dans un corps couché (< corps m, movement.separatePlayers : le debout recule seul) — null = hier au bit */, remisesPied: { elan: { recul: 3.5, lat: 1.5, vitesse: 4.0, patience: 4, sortieBut: { recul: 3, lat: 1.2, vitesse: 3.5 }, toucheLongue: { recul: 4 }, tirImmediat: { cone: 40 }, attente: { marche: 4 } /* (B2) le coup franc lancé / le corner dont la prise n'a pas fait partir le ballon se jouent dans l'image du contact d'élan (choix du porteur en urgence, tir au tick suivant) ; null : le double geste d'hier */ }, volee: { h: 1.0, avance: 0.45, lacher: 0.72 }, touche: { recul: 0.25 }, mur: { retard: 0.12, corps: 2.2, debout: 1.85, pieds: 0.25, rayon: 0.6, frein: 0.4 } }   /* (B4) corps : le ballon contre le mur — un vol qui passe à ≤ rayon m d'un homme du mur sous sa hauteur (corps m en l'air pendant la détente du clip, debout m sinon) est DÉVIÉ (vitesse × frein, relevé, événement dévié-mur) ; corps null : le mur traversé d'hier */,   /* (A9 ter) elan.sortieBut : la sortie de but LONGUE (style tactique ou pression) se prend au bout d'une course (recul m derrière le ballon, lat m côté pied faible, vitesse m/s) et se dégage dans l'image du contact ; elan.toucheLongue : la touche longue (tactique cpa.touche 'longue', tiers offensif) se lance au bout d'une course perpendiculaire (recul m, borné par le tablier) ; mur : les deux hommes du mur SAUTENT retard s après le contact du coup franc ('sautMur', acte planté) — chaque sous-clé null = hier au bit */   // (lot A9 bis) LES REMISES AU PIED : le coup franc et le corner se prennent AU BOUT d'une COURSE D'ÉLAN (referee.poserElan/elanStep/elanNow —
  ...
  attention: { tenue: 3, taux: 2 },   // LE LAPS D'ATTENTION PAR LA RÉACTION (246c, match-sim) : le distrait (concF < 1) garde, une tranche de tenue s sur (1 − concF) × taux, la cible du début de la tranche (zone ou homme) : il suit le jeu avec retard. MESURÉ 24 × 300 s, concentration 10 (concF 0,76, 48 % des tranches) : sur deux cibles, tirs 23/17 → 16/23 à possession égale ; sur les trois cibles, possession 52,4 → 49,3 et tirs 21/15 — le signe tient, la forme bouge : levier FAIBLE, à 48 graines au 246d ; à taux 1 (24 %) rien ne mord. null : concF n'est lu nulle part. (Le laps par DÉPLACEMENT du 246b est rejeté : 10 concédés c. 17.)
  placement: { bruit: 10, tenue: 3, zoneMorte: false },   // LE PLACEMENT EST UNE PRÉCISION (246, match-sim, bloc défensif) : le mauvais placeur tient son poste à bruit × (1 − posF) m à côté (positioning 10 : 1,2 m) ; zoneMorte false : la zone morte du slot (151, × (2 − posF)) ne lit plus la note — INVERSÉE, elle coûtait au bon placeur 5,6 pts de possession (47,5 c. 53,1, 12 × 300 s) ; true la rend ; placement null : l'hier au bit (bruit 0, zone morte 151)
  postesNommes: {},     // LES LOIS AU NOM DU POSTE (244b, formation.js — la grille 244a) : ALLUMÉE (false = l'indice d'hier au bit) — les pointes sont la strate ST et les AM larges (le dix axial reste entre les lignes), le pivot de la salida est le 6 de la grille (DM(C) sinon M(C) — hier l'intérieur gauche en 4-3-3), le dédoublement est celui du WB ou du D large (hier les indices 0/3, faux à trois ou cinq derrière). Mesuré (8 × 300 s) : 4-3-3 pivot M(C) 100 %, pertes 193 → 177 ; 3-5-2 : le dédoublement des DEUX pistons (hier D(CG) 18 + WB(G) 5 — un central débordait), appels 118 → 119, pertes 190 → 206 (Poisson) ; 4-2-3-1 : le dix sur la ligne 16 → 2 % des images, ses appels profonds 24 → 0, ceux du 9 23 → 35.
  ligneFermee: null,   // { seuil: 1, malus: 5, cloche: 4, retombee: 3, relu: 0.8 } l'allume — ÉTEINTE au 243 : posée et MESURÉE (6 × 300 s, tous à 50) : la part de passes jouées dans un corps ne bouge pas (5-6 %, 30 c. 70 en décisions pareil), la relecture ne tire que 18 fois / 30 min, et interdire la cloche par-dessus le marqueur collé au dos du receveur fait 324 → 483 pertes / 90 : ces passes sont le DERNIER RECOURS d'un porteur sans meilleure offre, pas une erreur de lecture — la réponse est structurelle (l'offre de passe, les couloirs), un nocif ne se livre pas. LA LIGNE FERMÉE (243, st.full — rondo.js choosePass, retour utilisateur « un adversaire sur la ligne de passe ») : la cloche par-dessus un corps exige ce corps à ≥ cloche m du passeur et ≥ retombee m du point visé (sinon la ligne est fermée, le candidat tombe) ; sous seuil m de marge, malus × (1 − marge/seuil) × decF — la note décisions refuse la ligne (1 à 50) ; relu : la ligne se RELIT à la frappe (strike-sim) — un défenseur entré pendant l'armé la ferme sous relu m × visionF, le geste finit en feinte, l'intention se re-choisit (hors urgence, centre, through, cloche). Absente : la cloche par-dessus n'importe quel corps, l'hier au bit
  hommeLibre: { seuil: 2.5, malus: 4 },
  passeMarque: { seuil: 4.5, malus: 5, remise: 8, remiseF: 0.35 },
  retournement: { rate: 4, cap: 1.75, max: 0.5, apres: 0.5, tour: 1.05 },   // max 1,2 → 0,5 (mesuré : l'attente coûtait des pertes, 447 c. 408/90 — le corps frappe à moitié tourné plutôt que de se faire prendre ; la remise du dos au jeu est la loi du 240)   // LE CORPS SE RETOURNE AVEC LE BALLON (240b, movement/rondo-sim/match-sim) : le porteur pivote à ≤ rate rad/s × accelF ; la passe à plus de cap rad dans son dos attend qu'il se retourne en conduisant vers le receveur (au plus max s ; pressé/jeté donnent) ; absente : le pivot à 540 °/s d'hier Le preneur du COUP D'ENVOI est dispensé dans sa fenêtre (loi 45 : l'engagement est une passe — 2,54 s c. 1,01 mesurés). tour : lancé (≥ 1,5 m/s), au-delà de tour rad entre la poussée voulue et son ÉLAN, le porteur garde le ballon au pied (porté) et ne pousse pas contre sa course (rondo-sim — 60°, le demi-cône du porté ; à l'arrêt la semelle tourne avec).
  chasseEta: null,   // LE PLUS RAPIDE AU BALLON (240c, match-sim) — ÉTEINTE : élu au temps d'arrivée (élan, accel × accelF, top × topF) au lieu de la distance ; mesuré à 12 graines SANS effet (l'élu prend le ballon 56 c. 60 %, durées égales) — un placebo ne se livre pas ; { top: 6.4 } l'allume
  epaule: { marge: 2, profond: 3, haut: 0.3 },
  pausa: { ttpPose: 1.5, ttpVif: 2.0, tenueBas: 1.25, tenueHaut: 0.8, garde: 0.7, max: 2.5, marge: 1.0, engages: 2, engage: 2, courseMin: 0.5, zone: [0.55, 0.88], rayon: 12, contact: 1.0, touche: 0.25, gainMin: 1.15 }, bouclier: { pression: 1.3, hysteresis: 0.4, dos: 0.3, face: 0.4, vMax: 4, pied: 1.2, arriere: 2, min: 0.8, max: 2.0, marge: 1.0, contact: 0.75, pousse: 0.6, pFaute: 0.3, cd: 3, grace: 0.8 }, ceremonie: { poignee: { rang: 0.42, pas: 0.8, decale: 2.6, espace: 0.75, tenue: 0.35, arrive: 0.3, trot: 2.4, marge: 1.2, patience: 60, avant: 1.2 }, salut: { tribune: 1, pas: 0.15, attente: 0.6, duree: 2.8, applaudir: 0.4 } },   // (A11 ter, ceremonie.js — note 381) L'AVANT-MATCH : la file des poignées avant le premier engagement (la rangée à rang m de la médiane, pas m entre les hommes ; la file à son pas, la poignée à l'arrivée (≤ arrive m) tenue s ; trot × marche vers les places, engagement quand tous à ≤ marge m ou patience s, avant s après) ; le salut au public au sifflet final (tribune ±1, échelonné de pas s après attente s, duree s pour la scène) ; null = l'hier au bit.   // (A10 ter, bouclier.js — note 379) LA TENUE DOS AU BUT : pressé dans le dos (côté but, cos ≥ dos, à ≤ pression m, derrière son regard cos ≤ −face, posé ≤ vMax, le ballon à ≤ pied m — bloqué sous la semelle s'il roule) sans appui DEVANT (une passe au-dessus de la barre qui n'est pas une remise à plus de arriere m en arrière), le porteur TIENT (arrêt, dos au presseur, ballon porté) de min à max s — jusqu'à l'appui, la POUSSÉE (collé < contact m plus de pousse s : pFaute × aggrF × rôle, cfg.loi12), le relâcher (pression + hysteresis) ; cd s entre deux ; null = l'hier au bit. LA PAUSA (253, pausa.js — la carte du book, Bible 07 §7 : 3-6 par match de 1,5-3,5 s, mesuré 0,5) : le porteur au calme (temps avant la pression ≥ seuil : ttpPose au tempo posé → ttpVif au tempo vif, × axe(tenue du rôle : tenueBas → tenueHaut) ÷ composureF), dans la zone [zone] du terrain, avec ≥ engages adversaires lancés vers le ballon (> engage m/s) et une course partenaire en cours (≥ courseMin s restantes) pas encore servable, TIENT : ni passe ni conduite (touche serrée touche) — jusqu'à servie / pression (ttp < garde) / expirée (max s, ou holdMax + 1) / course morte ; l'entrée exige hold ≤ holdMax − marge. L'événement pausa porte durée, issue, gain (≥ gainMin = valeur produite). null : l'adoption d'hier au bit
  priseAuPied: { r: 0.5, approche: 0.3 }, /* (333) la prise au pied (rondo-sim) : le ballon qui vient encore se laisse venir jusqu'au pied ; null : la prise à l'entrée du rayon d'hier */ corpsArret: { vMin: 1.5, seuil: 1, frein: 0.7 },   /* (326) LE CORPS S'ARRÊTE AVEC SON CONTRÔLE (corps-arret.js) : au contrôle, la vitesse du corps dans sa course se rapproche de celle du ballon amorti (frein × controlF) — il ne dépasse plus son ballon ; null : hier */ rythmeTouche: { tMin: 0.25, fuite: 0.2 }, porteRespire: { periode: 0.7, amp: 0.6, montee: 0.3, vMin: 1.5, vSprint: 5, sprintF: 1.4, sprintA: 1.8, tau: 0.1, foulee: { amp: 0.25, sprintA: 1.3, cote: 0.06 } /* (329) la respiration au pas : période de la foulée, serrée, du côté du pied fort — porte-respire.js */ }, repriseControle: { fenetre: 1.5, d: 0.8, revient: 0.5, avance: 0.25, traverse: 1, top: 6.0 },   /* (323-325) LA CONDUITE ET LE CONTRÔLE (l'atelier ?atelier=conduite, note 448) : la touche au rythme de la foulée (dribble.js : pas de re-touche d'un ballon qui s'échappe, tMin × dribbleLeadF), le porté respire (porte-respire.js : le servo avance et revient à la foulée, une touche par cycle, × gesteF, dribbleLeadF, l'espace), la reprise après le contrôle (movement.js : il va chercher À TRAVERS le ballon, à l'allonge dans la fenêtre) ; null : hier */ repliPasse: { recul: 2, marge: 2, portee: 30, tenue: 0.4, nMin: 1, nMax: 3 },   /* (318) LE REPLI SUR LA PASSE — repli-passe.js : pendant le vol d'une passe adverse vers mon dos, les n (axe transition 1-3) défenseurs qui arrivent le plus tôt au point goal-side de la destination y sprintent, après reaction × (2 − anticipF) × (2 − concF) ; null : hier */ interligne: { hz: 1, base: 6, gain: 9, kBas: 0.55, kHaut: 1.15, loMin: 5, loMax: 14, hiMin: 6, hiMax: 18, alarme: 16, rupture: 19, dureeRupture: 2 },   /* L'INTERLIGNE DÉRIVÉ ET SON POINT DE RUPTURE (274, interligne.js — Bible 10 §3.1-3.3, lot 4) : la portée d'intervention base + gain × (0,5 pace + 0,3 anticipation + 0,2 stamina) des milieux (10,5 au 50) × le mode de bloc (l'axe hauteurBloc : kBas, 1, kHaut), la cible [0,8 × ; 1 ×] bornée ; à hz la ligne du milieu tient ses cibles à [réf + lo ; réf + hi] au-dessus de la référence de l'unité arrière (273 — sans cfg.ligne, rien) ; l'écart des barycentres publie WARN (alarme) / BROKEN (rupture, tenue dureeRupture s → événement 'bloc' rupture). null : le bloc d'hier au bit */ ligne: { hz: 4, retard: 1.3, avant: 2, arriere: 5, seuil: 3, frein: 0.75, dureeFrein: 0.4, tol: 2, tenue: 0.5, montee: 4.8, recul: 3.9 },   /* LA LIGNE EST UNE LIGNE (273, ligne.js — Bible 03 §2.3 / T8, Bible 10 §10.1) : l'unité arrière relit à hz sa référence (le 2ᵉ plus reculé de ses cibles), chaque corps tient sa cible dans [réf − arriere ; réf + avant − retard_i] (retard_i = retard × (2 − anticipF) × (2 − posF)), le presseur hors de l'unité, le marqueur au contact retenu vers l'avant seulement ; ligne vécue cassée (desync > seuil) et ballon non couvert → le plus avancé qui monte freine (frein × pointe, dureeFrein s) ; le corps à plus de tol m de sa cible en profondeur rejoint la ligne au régime de la hauteur (montee en course avant, recul organisé, × topF — Bible 10 §3.4, lu par effort.js). null : la ligne d'hier au bit */ xg: { xMax: 35, cMax: 20, lambdaFar: 0.09, r0: 0.3, rEng: 0.55, pressT: 1.5, p0: 0.8, evCont: 1, plancher: 0.15, seuilMin: 0.01, d: { pied: 0.14, tete: -0.71, cf: -0.55, volee: -0.45, demiVolee: -0.2, omega: -2.2, press: -1.1, gk: 1.3, eta: 0.9, fin: 0.9 }, theta: { base: 0.01, temps: 0.012, doctrine: 0.03, fatigue: 0.01, pression: 0.02, role: 0.012 } },   /* LE xG EN FORME CLOSE ET LA PORTE (272, xg.js — Modèle 10 §1-§2) : le noyau de Sumpter clampé (xMax, cMax, λ_far au-delà), les corrections en log-odds d (tête / pied recentrés, occlusion Ω du cône — capsules r0 + rEng si engagé —, pression P à pressT, gardien avancé g/D, décentré η, finition ±0,45 dans le seul xG de décision), la porte xG_dec > EV_cont + Θ (evCont × pSucc × xG du point de passe ; Θ = theta.base − temps·ln κ − doctrine·(shotDoctrine − 0,5) + fatigue + pression·P − role·(arbitre.tir − 1)), lissée entre 0,5 et 1,5 (plancher). null : la zone de vérité du 232 (qualiteTir) au bit */   ballonFou: { v: 6.5, sigV: 0.35, vMin: 2, vMax: 12, sigElite: 45, sigMoyen: 75, sigMediocre: 100 },   /* LE BALLON FOU (271, st.full — doc fou.js : Bible 15 §3.4 la sortie stochastique du ballon disputé, D8b σ 75°, D9 le quart arrière). Après le pique, le tacle qui dégage, le contrôle manqué, le glissé gagné : la vitesse log-normale autour de v (σ_v, bornée), la direction N(0, σ_θ) enroulée autour de l'axe du dévieur, σ_θ par la qualité du point d'appui (élite / moyen / médiocre). null : les déviations d'hier au bit. */ temps: { bandes: { touche: [12.7, 21.7], 'sortie-de-but': [26.2, 36.7], corner: [30, 50], 'coup-franc': [25.8, 41.6] }, additionnel: { part: 0.35, serre: 60, ecartSerre: 1, min: 60, maxPart: 0.15 }, gk: { relache: 6, limite: 8 } },   /* LE TEMPS DU MATCH (270, st.full — doc temps.js : Bible 14 lot 3, Bible 16 lot 4, Loi 12.2). Les cérémonies dans la bande Opta de l'espèce × l'axe d'équipe gestionTemps (0,5 = le milieu), le temps additionnel part × arrêts + serre à la dernière période si |écart| ≤ ecartSerre (borné [min ; maxPart × période]), les huit secondes du gardien (relache, limite → corner). null : l'horloge d'hier au bit. */ nature: { specialiste: { k: 1.6, min: 0.1, max: 6 }, glisse: { battu: true, prise: 0.55, fuite: 0.5, p: 0.55, refusCd: 1, pFaute: 0.4, portee: 2.5 } },   /* LA NATURE DES GESTES (269, st.full — doc nature.js : Modèle 11 lot 2, Bible 14 T15bis, Bible 15 D20-D21). Le spécialiste : la tentative × exp(k · (flair centré + a_A)) ÷ sinh(k)/k (l'espérance à 1) — le meilleur 5 × le médian, le volume redistribué ; le glissé de dernier recours : battu seulement (le ballon fuit debout), au taux imposé p × aggrF × consigne duel (le refus coûte refusCd s), et sa faute (pFaute par glissé manqué à portée m — recuit pour 0,239). null : les gestes et le glissé d'hier au bit. */ noyau: { zone: 2.2, alpha: 2.05, rho: 0.95, T: 1.2, points: 9, secteur: 1.31, dMin: 2, dMax: 5, pressR: 6, sortieMax: 8, vSortie: 9, w: { mu: 0.6, dv: 0.15, psi: 0.5, side: 0.04, press: 0.25, goal: 0.9, nc: 0.4, attA: 0.35, attD: 0.35, sortie: 1.5, faute: 0 }, b: [0, -1.35, -2.84, -1.11, -2.02, 0.13, -3.41, -2.43], journal: false },   /* LE NOYAU COMMUN DE DUEL (268, st.full — doc noyau.js : Modèle 11 lot 1). Le take-on jugé au contact par un multinomial log-linéaire à huit issues (les intercepts b RECUITS DATÉ 268 sur 148 take-ons du moteur, 8 × 45 min : les parts marginales tombent sur celles du book — 44,6 / 11,6 / 3,2 / 10,0 / 3,3 / 24,1 / 0,7 / 2,3 ; b null = B_BOOK, les parts du book sans recuit) ; μ* le disque d'atteinte (alpha re-ajusté au profil du moteur, rho l'allonge, T l'horizon, points l'éventail), les poids w des features, la sortie près de la ligne (sortieMax). null : la géométrie d'hier au bit. */ piege: { p: 0.8, pas: 3, duree: 0.8, bande: 4, portee: 45, min: 14 }, selection: { survie: 'max', sigma: 0.45, lambda: 4.3, lambdaGk: 12.9, tauR: 0.25, fenetre: 0.25, vBallon: 9, lentAer: 1.3, vArr: 6.5, vArrAer: 9, avantage: 0.3, bloc: 0.06, pMin: 0.02, pMax: 0.995, p0: 0.8, poids: 2, rhoDef: 1.5, rhoOff: 0.5, dCourt: 15, dLong: 28, dArriere: 2, dSafe: 25, calage: { SHORT_GROUND: [1.15, 0.26], MID_GROUND: [1.1, 0.47], LONG_GROUND: [1.18, 0.37], CHANNEL: [1.18, 0.37], THROUGH: [1.18, 0.37], CHIP_THROUGH: [1.18, 0.37], SWITCH: [1.18, 0.37], CROSS: [1.18, 0.37], CUTBACK: [1.18, 0.37], LAY_OFF: [1.18, 0.37], ONE_TWO_RETURN: [1.18, 0.37], BACK_SAFE: [1.77, 0.4] } },   /* LA SÉLECTION CALIBRÉE DU PASSEUR (267, st.full — doc selection.js : Modèle 09 lot 2). La classe nommée (douze du book), P_succ prédit = P_rel × S (la survie de Spearman : σ, λ, la fenêtre de prise ; survie 'max' = le seul défenseur affecté, 'prod' = tous chassent) × PPCF_r (l'avantage du receveur servi) × P_ctrl (le 265), le calage log-odds par classe (calage[cls] = [α, β], AJUSTÉ DATÉ 267 sur 1 469 passes du monde d'hier (poids 0, graines 3, 7, 11, 13 ; σ, λ, fenêtre du book — la grille sur les données du moteur est plate, Modèle 07 test 10) : les classes à n ≥ 80 ont leur couple, les autres le couple global — β < 0,85 : la factorisation crue est omnisciente (test 2), la dette nommée ; null : α 0, β 1 — l'identité, test 9), le terme au barème poids × ρ(mentalité : rhoDef…rhoOff ; × decF) × (logit P̂ − logit p0). null : le barème d'hier au bit. */ ligneAccrochee: { consigne: { bas: 22, haut: 52 }, piege: 3, marge: { couvert: 8, entreDeux: 14, decouvert: 20 } /* (316) la ligne recule devant le dernier tiers — hier 2 / 6 / 12 : la ligne à 6 m du ballon, le porteur à sa hauteur en un dribble (15 échappées / match) */, plancher: 6 },   /* LA LIGNE ACCROCHÉE (280, ligne.js — Bible 10 §3.4) : la ligne arrière défendante vit à x_ligne = min(consigne, x_ballon − marge) — LIBRE à sa consigne (l'axe hauteurBloc : bas 22 → haut 52 m ± piege), ACCROCHÉE au porteur mètre pour mètre, la marge signée par l'état du porteur (couvert : le pas en avant ; découvert : le recul-frein) × (2 − anticipF). null : la ligne chaînée à 27 m du ballon d'hier, au bit */   rendezVous: { vMin: 1.5, vCourse: 7.5, iters: 6, armee: 0, vBallon: 12, vMax: 26, tMax: 2.5, repli: 0.6, kb: 0.8, biaisMax: 2.5 }, prefiltreTir: { dMax: 35, angleMin: 4, corps: 110, controle: 0 }, xt: { poids: 20, possession: 0.8, direct: 1.2 }, toucheRapide: { p: 0.45, min: 5, max: 9, pres: 7, libre: 18, marque: 2.5, gestion: 0.65, tard: 0.6 }, toucheAuPied: { court: 12, elev: 0.12, remiseLanceur: 0.35 }, attenteVivante: { amp: 1.2, periode: 3, loin: 5, appel: 2, rayon: 18, decroche: 2.5, serre: 4, colle: 0.6, lent: 0.8, vif: 1.2, loiTouche: 2.3 }, layoff: { kDev: 0.8, kV: 0.5, v0: 5, vRef: 5, margeMax: 4, kDev2: 2, kFoe: 1, foeRef: 3, face: 1, devMax: 135 }, /* (287) la une-touche jugée par son angle : la dispersion × (1 + kDev × dev/90°) × (1 + kV × (arr − v0)/vRef) ÷ layoffF, le barème de la cible (l'angle, la cible marquée, de face), l'angle impossible se contrôle */ toucheEngage: null, /* (286) LA TOUCHE QUI ENGAGE — loi NOMMÉE, ÉTEINTE par défaut (null = le pivot d'hier au bit) : après un contrôle et sans presseur, pas de demi-tour pendant commit × (2 − decF) × tempo s (la passe dans le dos coûte malus, la conduite garde son cap à ± angle). RÉFUTÉE par la mesure (3 × 600 s et 4 × 90 min) : les demi-tours après contrôle ne baissent pas (30 → 29 % en course, 38 → 33 % posé) et les pertes montent de 12 % — la conduite forcée vers l'avant se perd dans les jambes ; un projet l'allume : { commit: 0.5, angle: 100, malus: 8, presse: 2.5, lent: 1.2, vif: 0.8 } */ /* (285) l'attente vivante : le micro-déplacement seedé autour du poste (× workF × tempo), le décrochage des receveurs et le serrage des marqueurs aux dernières secondes */ /* (284) la touche rapide (la queue basse de la bande Opta, moyenne gardée) et la touche au pied (le jet court tendu ; la remise de tête ne revient au lanceur qu'à remiseLanceur × (2 − decF)) */ /* (283) la valeur de position xT au barème de la passe (Modèle 06 §3, §8.1) : poids × P_succ × ΔV × visionF × axe(style) */ /* (282) le pré-filtre de la porte du tir (Modèle 10 §1.4) : distance < 35 m, angle visible > 4°, corps à < 110° × pivotF, contrôle ≥ 0 s */   /* LA PASSE AU RENDEZ-VOUS (281, rendezvous.js — Modèle 09 §3) : le receveur en course (≥ vMin) reçoit au point fixe r + v T, T = vol (la balistique réelle, iters), vitesse du coureur bornée vCourse × topF, la passe trop forte (> vMax) ou trop longue (> tMax) se replie (× repli), le biais de sécurité vers l'avant kb × ℓ × σψ (≤ biaisMax). null : la mène d'hier au bit */   remisePostes: { appuis: 2, recul: 5, cote: 6 },   /* LES POSTES DE LA REMISE (281, match-sim) : à la remise (coup franc, sortie de but, corner sans plan…), l'équipe qui remet tient ses postes de formation ancrés au ballon ; appuis corps viennent en soutien (recul m derrière, cote m de côté) ; le preneur a son métier. null : hier au bit (tous au ballon) */   arretControle: { fenetre: 3 },   /* L'ARRÊT AU JOURNAL (279, match-sim) : le tir adverse contrôlé par le gardien dans les fenetre s est un 'arrêt' mode 'controle' (le taux d'arrêt se mesure entier). null : le journal d'hier au bit */   repertoire: { vMax: 35.5, boutPortant: 8, bride: 0.9, anisoRef: 2 },   /* LE RÉPERTOIRE DU BOOK (279, repertoire.js — Modèle 10 §3.1) : les vitesses de frappe sourcées (placé 20, instep 28, enroulé 24, pointu 16, volée 26, tête 13) × powF (shotPower), la bride du bout portant (instep à D < boutPortant × bride), vMax par attribut (vMax × vMaxF ∈ [33 ; 38]) dans f_v, les colonnes de dispersion du geste sur l'ellipse (aniso / anisoRef). null : les vitesses d'hier, au bit */   ellipse: { sigma0: 4.0 /* (335) 2,0 → 4,0 : 60 % de tirs cadrés (réel 33-40), 9 % à côté (réel ~35) — avec le choix en valeur attendue, les buts restent à 4,2 (4,65 sans) */, aniso: 4.5, corps: 0.9, rhoPsiTheta: 0.2, rhoThetaV: -0.35, queue: 1.6, biaisPsi: 1.5, sens: 1, xiMax: 3.5, vPlafond: 33, vPlancher: 10, nominal: { iters: 7, elevMax: 0.6 } },   /* L'ELLIPSE DE FINITION (278, ellipse.js — Modèle 03 §5) : l'erreur du tir tirée UNE fois à la frappe, sur le point visé du 277 — (Δψ, Δθ) normale bivariée (ρψθ), σψ = sigma0° × les facteurs du 258, σθ = aniso × σψ, f_corps = 1 + corps (1 − cos Δφ) ; la NOMINALE intégrée (nominal : predictPath jusqu'au plan, iters bissections de θ sur la hauteur visée puis ψ corrigé sur le z franchi — la traînée et l'effet du geste compris ; null : la résolution dans le vide d'hier) ; ln v corrélé à Δθ (rhoThetaV < 0 : précipitée = levée et molle), queue basse × queue, tronqué [vPlancher ; vPlafond] ; μψ = biaisPsi° × P × sens vers l'extérieur du pied ; vraies queues (inversion, |ξ| ≤ xiMax). null : les trois gauss d'hier, au bit */   visee: { bord: 0.1, yMin: 0.1, pressT: 1.2, pression: { p0: 0.7, largeur: 0.3 }, facteurs: { gAvance: 3, lucarneAvance: 1.5, lobAvance: 2, gEngage: 1.5, contrePied: 1.3, angleFerme: 0.6, premierPoteau: 2, ouvertFerme: 0.7, boutPortant: 8, axialPres: 2 } },   /* LE POINT VISÉ (277, visee.js — Modèle 10 §3.4, Miss It Like Messi) : le tir vise un tirage dans le mélange à neuf modes du book (lucarne / bas / mi-hauteur côté ouvert, bas / mi côté fermé, sous la barre, axial bas, premier poteau, vers le cadre) — le côté ouvert lu du décentrage du gardien, les poids conditionnés (composure → lucarne, flair et gardien engagé → contre-pied, gardien avancé → lucarne et lob, angle fermé → premier poteau, bout portant → axial), la pression (P au-delà de p0, sur largeur) effondre vers « le cadre » ; la hauteur visée voyage avec l'espèce, la dispersion du 258 s'ajoute. null : le coin loin du gardien d'hier au bit */ enveloppe: { kD: 0.0108, tauR: 0.2, tauA: 0.2, vd: 4.4, r0: 1.5, rMax: 1.9, zc: 0.95, ellipse: 0.8, seuilBloc: 0.25, blocF: 1, marge: 1.25, tirage: false, dtOccl: 0.12, occlMax: 0.25, beta: { b0: 0, b1: 3.2, b2: 1.1, b3: 0.4, b4: 1.0, b5: 1.6 } },   /* LE GARDIEN À ENVELOPPE CONTINUE ET PSxG (276, enveloppe.js — Modèle 10 §6.2-6.4, lot 4) : t_f = (e^{kD D} − 1)/(kD v0) ; t_disp = t_f − tauR (1 − 0,5 anticipation) − occlusion ; R(t) = r0 + min(vd [t − tauA (1 − e^{−t/tauA})], rMax × keeping) — r0 1,5 : le demi-corps et le bras tendu (le book ne le chiffre pas), rMax 1,9 le haut de sa plage, marge 1,25 le gant et le ballon au-delà du centre de masse ; calé sur ce moteur dont le tireur vise le poteau loin du gardien (ρ p50 3,9 m — le point visé du § 3.4 est la dette) ; l'ellipse (b = ellipse × a, centre zc) ; sous seuilBloc on bloque (r0 × blocF) ; p_save = σ(beta) : UN tirage par tir (tirage, au flux seedé — le seul aléa non physique de la chaîne, § 6.4 ; false : la géométrie ρ ≤ R × marge) décide si le gardien y est, la physique du gant résout ; PSxG = 1 − p_save journalisé (événement 'enveloppe', st.psxg). null : le seuil d'envergure d'hier (diveReach) au bit */ blocPercu: { base: 0.22, parM: 0.008, anticipation: 0.15, fatigue: 0.3, kx: 1, max: 12, extrap: 6 },   /* LE BLOC QUI PERÇOIT (275, bloc-percu.js — Bible 10 §4.4, lot 1 ; Modèle 04) : la cible de bloc de chaque posté se dérive de SA croyance du ballon (cfg.croyance) relue à sa latence de déclenchement (base + parM × d, × (1 − anticipation × (r − 0,5)), × (1 + fatigue × (1 − stamina))) ; entre deux relectures l'ancre crue avance à la vitesse crue (extrap m au plus) ; le slot se décale de la réponse du bloc (kx en régime accroché, 0 au plafond ; le gain latéral bloc.lateral), borné à max m. null : l'omniscience du bloc d'hier au bit */ interligne: { hz: 1, base: 6, gain: 9, kBas: 0.55, kHaut: 1.15, loMin: 5, loMax: 14, hiMin: 6, hiMax: 18, alarme: 16, rupture: 19, dureeRupture: 2 },   /* L'INTERLIGNE DÉRIVÉ ET SON POINT DE RUPTURE (274, interligne.js — Bible 10 §3.1-3.3, lot 4) : la portée d'intervention base + gain × (0,5 pace + 0,3 anticipation + 0,2 stamina) des milieux (10,5 au 50) × le mode de bloc (l'axe hauteurBloc : kBas, 1, kHaut), la cible [0,8 × ; 1 ×] bornée ; à hz la ligne du milieu tient ses cibles à [réf + lo ; réf + hi] au-dessus de la référence de l'unité arrière (273 — sans cfg.ligne, rien) ; l'écart des barycentres publie WARN (alarme) / BROKEN (rupture, tenue dureeRupture s → événement 'bloc' rupture). null : le bloc d'hier au bit */ ligne: { hz: 4, retard: 1.3, avant: 2, arriere: 5, seuil: 3, frein: 0.75, dureeFrein: 0.4, tol: 2, tenue: 0.5, montee: 4.8, recul: 3.9 },   /* LA LIGNE EST UNE LIGNE (273, ligne.js — Bible 03 §2.3 / T8, Bible 10 §10.1) : l'unité arrière relit à hz sa référence (le 2ᵉ plus reculé de ses cibles), chaque corps tient sa cible dans [réf − arriere ; réf + avant − retard_i] (retard_i = retard × (2 − anticipF) × (2 − posF)), le presseur hors de l'unité, le marqueur au contact retenu vers l'avant seulement ; ligne vécue cassée (desync > seuil) et ballon non couvert → le plus avancé qui monte freine (frein × pointe, dureeFrein s) ; le corps à plus de tol m de sa cible en profondeur rejoint la ligne au régime de la hauteur (montee en course avant, recul organisé, × topF — Bible 10 §3.4, lu par effort.js). null : la ligne d'hier au bit */ xg: { xMax: 35, cMax: 20, lambdaFar: 0.09, r0: 0.3, rEng: 0.55, pressT: 1.5, p0: 0.8, evCont: 1, plancher: 0.15, seuilMin: 0.01, d: { pied: 0.14, tete: -0.71, cf: -0.55, volee: -0.45, demiVolee: -0.2, omega: -2.2, press: -1.1, gk: 1.3, eta: 0.9, fin: 0.9 }, theta: { base: 0.01, temps: 0.012, doctrine: 0.03, fatigue: 0.01, pression: 0.02, role: 0.012 } },   /* LE xG EN FORME CLOSE ET LA PORTE (272, xg.js — Modèle 10 §1-§2) : le noyau de Sumpter clampé (xMax, cMax, λ_far au-delà), les corrections en log-odds d (tête / pied recentrés, occlusion Ω du cône — capsules r0 + rEng si engagé —, pression P à pressT, gardien avancé g/D, décentré η, finition ±0,45 dans le seul xG de décision), la porte xG_dec > EV_cont + Θ (evCont × pSucc × xG du point de passe ; Θ = theta.base − temps·ln κ − doctrine·(shotDoctrine − 0,5) + fatigue + pression·P − role·(arbitre.tir − 1)), lissée entre 0,5 et 1,5 (plancher). null : la zone de vérité du 232 (qualiteTir) au bit */   ballonFou: { v: 6.5, sigV: 0.35, vMin: 2, vMax: 12, sigElite: 45, sigMoyen: 75, sigMediocre: 100 },   /* LE BALLON FOU (271, st.full — doc fou.js : Bible 15 §3.4 la sortie stochastique du ballon disputé, D8b σ 75°, D9 le quart arrière). Après le pique, le tacle qui dégage, le contrôle manqué, le glissé gagné : la vitesse log-normale autour de v (σ_v, bornée), la direction N(0, σ_θ) enroulée autour de l'axe du dévieur, σ_θ par la qualité du point d'appui (élite / moyen / médiocre). null : les déviations d'hier au bit. */ temps: { bandes: { touche: [12.7, 21.7], 'sortie-de-but': [26.2, 36.7], corner: [30, 50], 'coup-franc': [25.8, 41.6] }, additionnel: { part: 0.35, serre: 60, ecartSerre: 1, min: 60, maxPart: 0.15 }, gk: { relache: 6, limite: 8 } },   /* LE TEMPS DU MATCH (270, st.full — doc temps.js : Bible 14 lot 3, Bible 16 lot 4, Loi 12.2). Les cérémonies dans la bande Opta de l'espèce × l'axe d'équipe gestionTemps (0,5 = le milieu), le temps additionnel part × arrêts + serre à la dernière période si |écart| ≤ ecartSerre (borné [min ; maxPart × période]), les huit secondes du gardien (relache, limite → corner). null : l'horloge d'hier au bit. */ nature: { specialiste: { k: 1.6, min: 0.1, max: 6 }, glisse: { battu: true, prise: 0.55, fuite: 0.5, p: 0.55, refusCd: 1, pFaute: 0.4, portee: 2.5 } },   /* LA NATURE DES GESTES (269, st.full — doc nature.js : Modèle 11 lot 2, Bible 14 T15bis, Bible 15 D20-D21). Le spécialiste : la tentative × exp(k · (flair centré + a_A)) ÷ sinh(k)/k (l'espérance à 1) — le meilleur 5 × le médian, le volume redistribué ; le glissé de dernier recours : battu seulement (le ballon fuit debout), au taux imposé p × aggrF × consigne duel (le refus coûte refusCd s), et sa faute (pFaute par glissé manqué à portée m — recuit pour 0,239). null : les gestes et le glissé d'hier au bit. */ noyau: { zone: 2.2, alpha: 2.05, rho: 0.95, T: 1.2, points: 9, secteur: 1.31, dMin: 2, dMax: 5, pressR: 6, sortieMax: 8, vSortie: 9, w: { mu: 0.6, dv: 0.15, psi: 0.5, side: 0.04, press: 0.25, goal: 0.9, nc: 0.4, attA: 0.35, attD: 0.35, sortie: 1.5, faute: 0 }, b: [0, -1.35, -2.84, -1.11, -2.02, 0.13, -3.41, -2.43], journal: false },   /* LE NOYAU COMMUN DE DUEL (268, st.full — doc noyau.js : Modèle 11 lot 1). Le take-on jugé au contact par un multinomial log-linéaire à huit issues (les intercepts b RECUITS DATÉ 268 sur 148 take-ons du moteur, 8 × 45 min : les parts marginales tombent sur celles du book — 44,6 / 11,6 / 3,2 / 10,0 / 3,3 / 24,1 / 0,7 / 2,3 ; b null = B_BOOK, les parts du book sans recuit) ; μ* le disque d'atteinte (alpha re-ajusté au profil du moteur, rho l'allonge, T l'horizon, points l'éventail), les poids w des features, la sortie près de la ligne (sortieMax). null : la géométrie d'hier au bit. */ piege: { p: 0.8, pas: 3, duree: 0.8, bande: 4, portee: 45, min: 14 }, selection: { survie: 'max', sigma: 0.45, lambda: 4.3, lambdaGk: 12.9, tauR: 0.25, fenetre: 0.25, vBallon: 9, lentAer: 1.3, vArr: 6.5, vArrAer: 9, avantage: 0.3, bloc: 0.06, pMin: 0.02, pMax: 0.995, p0: 0.8, poids: 2, rhoDef: 1.5, rhoOff: 0.5, dCourt: 15, dLong: 28, dArriere: 2, dSafe: 25, calage: { SHORT_GROUND: [1.15, 0.26], MID_GROUND: [1.1, 0.47], LONG_GROUND: [1.18, 0.37], CHANNEL: [1.18, 0.37], THROUGH: [1.18, 0.37], CHIP_THROUGH: [1.18, 0.37], SWITCH: [1.18, 0.37], CROSS: [1.18, 0.37], CUTBACK: [1.18, 0.37], LAY_OFF: [1.18, 0.37], ONE_TWO_RETURN: [1.18, 0.37], BACK_SAFE: [1.77, 0.4] } },   /* LA SÉLECTION CALIBRÉE DU PASSEUR (267, st.full — doc selection.js : Modèle 09 lot 2). La classe nommée (douze du book), P_succ prédit = P_rel × S (la survie de Spearman : σ, λ, la fenêtre de prise ; survie 'max' = le seul défenseur affecté, 'prod' = tous chassent) × PPCF_r (l'avantage du receveur servi) × P_ctrl (le 265), le calage log-odds par classe (calage[cls] = [α, β], AJUSTÉ DATÉ 267 sur 1 469 passes du monde d'hier (poids 0, graines 3, 7, 11, 13 ; σ, λ, fenêtre du book — la grille sur les données du moteur est plate, Modèle 07 test 10) : les classes à n ≥ 80 ont leur couple, les autres le couple global — β < 0,85 : la factorisation crue est omnisciente (test 2), la dette nommée ; null : α 0, β 1 — l'identité, test 9), le terme au barème poids × ρ(mentalité : rhoDef…rhoOff ; × decF) × (logit P̂ − logit p0). null : le barème d'hier au bit. */
  interception: { tauR: 0.25, mort: 0.15, passeur: true },   // L'INTERCEPTION NON OMNISCIENTE (266, st.full — doc interception.js : Modèle 07 lot 2). Pendant tauR × (2 − anticipF) s après le départ d'une passe, le défenseur n'a pas lu le départ (sa cible reste celle du ballon au départ) ; ensuite il vise le ballon tel qu'il le CROIT (croyance.js) — l'écart cru − vrai se retranche aux cibles des métiers relatifs au ballon (press, intercept, cover), sous mort m il ne bouge rien ; et le passeur (passeur) projette les défenseurs là où il les croit dans sa course de refus. null : l'omniscience d'hier au bit.
  passe: { d0: 0.85, dLoss: 2.8, kappa: 3.0, chiAer: 1.55, hAer: 0.6, pressT: 1.5, tClean: 0.25 /* (297) LE CONTRÔLE PROPRE AU LIVRE : le budget 0,25 s du Modèle 09 § 6.1 (0,4 au moteur — le presseur arrivait pendant le contrôle : ~50 réceptions contestées par équipe) ; mesuré avec la remise : contrôles manqués 7,4 → 5,5 %, contestées perdues 30 → 18. 0,4 : hier */, dHeavy: 0.9, dHeavyMax: 2.0, tHeavyMin: 0.55, tHeavyMax: 0.8, contourne: 0.5, edgeF: 1, sigma0: 1.4, mcCourt: 0.8, mcLong: 1.3, mcCentre: 1.7, dCourt: 12, dLong: 25, kappaP: 1.2, fDist: 0.012, dRef: 12, muV: 0, muP: 0, sigV: 0.06 },
                          // LA PASSE QUI SE MANQUE À LA BONNE DISTANCE (265, st.full — doc reception.js : Modèle 09 lot 1).
                          // (1) la réception à quatre issues : d_touch = d0 (v_rel/10)^0,8 (1 + 0,5 P) χ_aérien (hAer) ÷
                          // controlF ; P = 1 − TTP/pressT (le temps d'arrivée au BALLON, + contourne s pour le presseur derrière le corps) ; manqué p = 1 − e^{−(d/dLoss)^kappa} ; contesté si TTP < budget
                          // (propre tClean sous dHeavy, lourde tHeavyMin…tHeavyMax jusqu'à dHeavyMax) → 50/50 × chargeF
                          // (edgeF) ; propre : le porté protégé tClean. (2) l'erreur de geste : σ = sigma0° × note × classe
                          // (mcCourt < dCourt, mcLong > dLong ou piqué / cloche, mcCentre) × (1 + kappaP × composureF × P) × (1 + fDist (d − dRef)⁺) ; le dosage e^{muV + muP P + sigV ξ} (le sous-dosage du book, muV −0,05 / muP −0,10, est ÉTEINT : le receveur d'hier ne vient pas au-devant d'une balle courte — mesuré 9 → 14 % de passes perdues avant sa touche ; dette nommée). Réel visé : 5-15 yd 88-92 %, 15-30 82-87, 30+ 55-65, contrôle manqué 2-4 %. null : la réception et le σ d'hier au bit.
  ...
  effort: { saut: 5, sautV: 9, actifDur: 2.5, tolOff: 1.2, vEnt: 1.4, vEntDef: 1.8, epsEnt: 0.45, vActif: 4.2, epsActif: 0.6, gRecup: 12, rayonTrans: 20, chaud: 10, tAtt: 2.5, appelPortee: 22, fenetre: 5, repliSprint: 5, vRecup: 5.0, epsRecup: 0.7 },
                          ...
                          // (1) les trois régimes du SUIVEUR (support/mark/cover) — le SAUT du slot (≥ saut m d'un coup, à
                          // plus de sautV m/s) déclenche le COULISSEMENT ACTIF (vActif, epsActif) pour actifDur s (× workF ;
                          // marqueur × axe marquage 0,7…1,3) ou jusqu'à l'arrivée (tolOff) ; sinon on ENTRETIENT : le slot
                          // suivi à sa vitesse (× 1,15 + 0,4), plancher vEnt avec le ballon (la marche), vEntDef sans lui
                          // (la marche rapide du bloc) — quand l'intention parle, l'allure d'hier se tait ; et l'on RÉCUPÈRE au plafond du
                          // métier au-delà de gRecup, en transition défensive à moins de rayonTrans (× axe transition
                          // 0,6…1,4) du ballon, dans la fenêtre de pressing des miens ou à moins de chaud m du ballon ;
                          // (2) le rayon d'ATTEIGNABILITÉ du presseur : hors fenêtre et hors chasse, une cible à plus de tAtt s
                          // (× axe pressing 0,7…1,3 × rôle press 0,8…1,2 × workF) se FERME au régime actif ; (3) l'APPEL ne se
                          // tire qu'à appelPortee m du ballon ; (4) le REPLI est un sprint de transition : passé repliSprint s
                          // depuis la perte, celui qui rentre encore récupère à vRecup (epsRecup), il ne sprinte plus. Le score
                          // ne branche rien : le coach déplace les axes (Bible 16).
                          // Réel visé (Référentiel 05 §3) : marche 30 % / jog 34 % / course 30 % / HI < 7 % / sprint < 2 % de la
                          // distance, 10,5 km. null : la table des métiers d'hier au bit.
  locomoteur: { plein: ['carry', 'receive'] /* (310) la poussée pleine du porteur et du receveur, la demande comme plafond — locomoteur.pasLoco ; absent : hier */, freinEffort: 2 /* (307) le frein des rôles SANS ballon suit ε² (locomoteur.pasLoco) : freinages < −3 m/s² 4,2 → 2,4 par joueur-minute (réel 0,86-1,17) ; absent : hier */, v0: 8.8, tau: 1.17, f0Min: 5.0, f0Max: 10.2, epsRupture: 1.0, epsPorteur: 0.85, epsPress: 0.85, epsGardien: 0.85, epsMarche: 0.45, epsBloc: 0.55, epsAttaque: 0.75, attaqueX: 0, vLent: 1.5, aLent: 4, touche: 0.8, dMax: 6.0, vBrk: 2.0, seuilFrein: 3.0, roule: 1.5, vCP: 5.5, dPrime: 350, tauRec: 280, lamV: 0.07, lamTau: 0.22, lamD: 0.18, wBurst: 0.2, seuilEv: 0.2 },   // LE PROFIL LOCOMOTEUR ET LE BUDGET DE COURSE (260, locomoteur.js / movement.js — Modèle 02, Référentiel 05 : le constat n° 1 des Bibles) : a = ε (V − v)/τ avec V₀ = v0 × topF, τ = tau ÷ accelF, F₀ borné [f0Min ; f0Max] ; ε par métier (rupture / porteur / pressing-chasse-réception / gardien / marche / bloc, et epsAttaque pour le soutien de l'équipe en possession au-delà d'attaqueX m de la médiane) ; la petite demande (< vLent m/s) marche à aLent m/s² ; la touche poussée du porteur × touche (le corps qui démarre lentement ne rattrape pas la poussée d'hier) ; le freinage −dMax × min(1, v/vBrk) au-delà de seuilFrein m/s d'écart, le roulé −roule m/s² en dessous (freiner est une intention aussi) ; le réservoir W′ (p.wp) se vide au-dessus de vCP m/s (D′ dPrime m ÷ stamF), se remplit en tauRec s, dégrade τ (lamTau) et le freinage (lamD) avant la pointe (lamV), refuse la pointe sous wBurst (p._paceRefus) ; événement budget sous seuilEv. null : l'accélération constante cfg.accel et les pointes gratuites d'hier au bit
  familiarite: { sync0: 0.12, sync1: 0.40, motif: 0.7, etaMotif: 0.5, chocPosture: 0.7, a: 0.55, tFast: 90, tSlow: 1500 },   // LA FAMILIARITÉ, LE MÉCANISME RELATIONNEL (254, familiarite.js — Modèle 14 §7, Référentiel 13) : η par équipe (la moyenne des squads[i].familiarite, défaut 1 : rodée), choqué à chocPosture par un changement de posture du coach et rétabli sur deux branches (a × e^(−t/tFast) + (1 − a) × e^(−t/tSlow)) ; Φij = √(fi fj) × η^etaMotif conditionne l'un-deux et le troisième homme (P × Φ^motif) ; la synchronie de la ligne au piège est σ_sync = sync0 + sync1 (1 − η) s (chaque corps part avec son retard |gauss| × σ — à η 0,4 : 0,36 s, ≈ 2,5 m de ligne brisée). Aucun canal ne touche la réussite d'une action (le référentiel l'interdit). null : la ligne parfaitement synchrone et les motifs pleins d'hier au bit   // LE PIÈGE DU HORS-JEU, LA LIGNE SYNCHRONE (255, piege.js — Bible 03 T3/T3b, Bible 10 §10.2 : ce n'est pas la hauteur qui provoque le hors-jeu, c'est la synchronie) : quand le porteur adverse ARME une passe entre min et portee m du but défendu, la ligne arrière (les corps de champ à ≤ bande m de l'avant-dernier) monte ENSEMBLE de pas × axe(piege) m pendant duree s, avec P = p × axe(piege) par armé au flux seedé — la photo de la Loi 11 (259) prend les corps restés sur l'épaule ; l'espace derrière la ligne est le prix, dans le même mécanisme. L'axe tactique piege garde son +3 m de ligne postée (149). null : la ligne d'hier au bit
  horsJeu: { tronc: 0.2, foulee: 0.3, freq: 2.2, audela: 4, epaule: true, pres: 5, recul: 2.5, espace: 12, axe: 14, portee: 28, cd: 12, tente: 1.2 },   // L'ORTEIL ET LA COURSE QUI TRAVERSE (259, offside.pointCorps / strike-sim la photo / match-sim l'appel — la carte du book, Modèle 12 §1.2-§2, Bible 09 §4.1 : réel 3,1-4,5 hors-jeu par match, le 9 pris 0,3-1,2 fois par 90 ; hier 0,75 par match). La photo de la Loi 11 lit LA PARTIE DU CORPS la plus avancée : une capsule — tronc m + l'extension du pied avant foulee × min(1, v/8) × |sin(2π freq t)| pour celui qui court vers le but (l'attaquant vers le but adverse, le défenseur vers le sien) ; le cerveau du passeur, lui, juge le centre : l'écart fabrique le hors-jeu d'un orteil. Et la course de l'appel ne s'arrête plus à 0,15 m de la ligne (la ligne collante, l'erreur 1 du book) : elle la TRAVERSE de audela m — le passeur sert le coureur tant qu'il est en jeu, la frappe le juge. Et L'APPEL DE L'ÉPAULE (epaule, Bible 09 §4.1 Van Nistelrooy : « rester hors jeu jusqu'à ce que le ballon soit joué ») : la pointe posée à ≤ pres m de la ligne, dans l'axe (|z| < axe), avec espace m derrière la ligne, part quand le porteur est prêt (posé) — hors créneau d'équipe, hors cooldown, hors couloir (le porteur à ≤ portee m ; cadence cd s ÷ otbF) : le passeur la sert tant qu'elle est en jeu, la photo la prend d'un orteil ; la pointe posée prend l'épaule avec recul m d'élan EN PLUS de sa marge de rôle (240d : le 9 haut 0,6, le polyvalent 2, le décrocheur 6 — le rôle survit). Et LA TENTATIVE (tente, Loi 11 (b) : interférer) : le photographié qui arrive à ≤ tente m du ballon en vol est sifflé sans attendre son pied. null : la ligne collante et la photo au centre d'hier au bit   // LA POINTE SUR L'ÉPAULE (240d, match-sim) : la cible d'une pointe posée remonte à marge × axe(profondeur : décrocheur ×profond, 9 haut ×haut) m de la ligne de hors-jeu ; absente : le slot qui traîne d'hier   // LE MARQUÉ NE SE JOUE QU'EN REMISE (240a, choosePass) : sous seuil m de liberté, malus × (1 − liberté/seuil) × visionF × axe style ÷ controlF du receveur, ÷ remiseF s'il a une remise (coéquipier libre à ≤ remise m, couloir dégagé) ; absente : l'hier au bit   // L'HOMME LIBRE (233, choosePass) : malus × (1 − liberté projetée du receveur / seuil) × visionF × axe(style) — le marqué se refuse (18 % des passes, interceptées à 27-36 %). null : le barème linéaire d'hier
  couvert: { pres: 2, libre: 3.5, face: 0.3, monte: 3, recule: 5, tau: 0.2, lecture: true },   // lecture (246b) : le bloc qui lit monte PLUS TÔT (tau ÷ anticipation moyenne), pas PLUS (l'amplitude × anticipMoy d'hier faisait concéder 12-13 tirs c. 6-8 à anticipation 90 ; corrigé : 3 c. 12 à 10, 12 × 300 s)   // BALLON COUVERT / DÉCOUVERT (236, couvert.js) : porteur cadré (≤ pres m ou dos au jeu) → la ligne monte de monte m × axe(hauteurBloc, 0,6, 1,4) ; libre (> libre m, face au jeu) → recul-frein de recule m × axe(1,4, 0,6) ; × anticipation moyenne de la ligne ; lissé (tau s). null : la ligne indifférente d'hier
  ...
  couloirs: { max: 2, tenue: 2, demi: 0.6, remplir: true, relais: null, ligne: null, bande: 8, pointesLibres: true, engagement: 2.5 },   // LES CINQ COULOIRS (241, st.full — couloirs.js) : jamais plus de max cibles de la structure d'attaque (porteur, postés, soutiens) dans le même couloir vertical (largeur / 5) ; la cible en trop passe au couloir voisin libre (côté du ballon à égalité — contre le ballon, la fixation du 98 mourait : 0 bascule —, un demi-espace VIDE attire de demi couloir) et y TIENT tenue s (hystérésis) ; remplir : un demi-espace voisin resté vide l'image d'avant attire le second corps d'un couloir occupé (le relais dans chaque demi-espace) ; relais ({ marge: 1.5 } l'allume — ÉTEINT (null) au 241 : cibles dans les deux demi-espaces 100 % mais corps à ≤ 15 m derrière le ballon 60 → 60-66 % selon les variantes, ± 3 pts de bruit, un placebo ne se livre pas) : L'INTÉRIEUR TIENT SON DEMI-ESPACE — un poste de la ligne des milieux dont le spot de base tombe dans un demi-espace y reste borné en possession (marge m du bord ; le coulissement latéral vidait le demi-espace opposé) ; l'élection d'un relais à ballon + 6 m a été essayée et rejetée (réussite 74 → 71) ; ligne / bande : pas plus de ligne cibles dans la même bande de bande m sur l'axe d'attaque — l'excédent recule d'une bande vers le ballon (l'échelonnement), sous la ligne de hors-jeu, même tenue — ÉTEINTE (null) au 241 : effet nul mesuré (lignes à ≥ 4 corps 16,5 → 18,6 % à 12 graines), un placebo ne se livre pas ; ligne: 3 l'allume ; pointesLibres : les pointes (premierOffensif) ne passent pas au registre — elles vivent sur la ligne (Loi 11, 240d) et leurs appels profonds tombaient de 26 % quand le registre les sortait du couloir du ballon (garde 231) ; engagement : le registre dort engagement s après le coup d'envoi (la chorégraphie de la loi 45 — le soutien du preneur débordait à 13 m, prise → passe 1,0 → 5,7 s). Le registre vaut pour les DIX (la règle de Guardiola ; la restriction à la structure d'attaque affaiblissait les couloirs : 30 → 39 %). Absente : l'empilement au ballon d'hier (un couloir à ≥ 3 corps sur 50 % des images)
  offre: null,   // { cone: 12, pas: 1.75, max: 3.5 } — ÉTEINTE au 241 : mécanisme prouvé (fixture : 23 évaluations dans l'ombre, 23 points nets), flux NUL (soutiens élus dans l'ombre 32 → 31 %, passes dans l'ombre 16 → 15, réussite 74 → 73 à 12 graines) — un placebo ne se livre pas ; à rallumer quand un flux la lira. L'OFFRE DE PASSE (241b, st.full — seMontrer) : un soutien dans le cône d'ombre (± cone °) d'un défenseur entre lui et le porteur se décale de pas puis max m × offBallF (déplacement sans ballon, note 50 = identité) vers la ligne nette. Absente : le test de couloir et les pas 2,5 / 5 d'hier (67)
  ...
  porteePasse: { max: 45, leve: 32, retrait: { marge: 6, tous: true } /* (313, 313b tous : par tous les chemins) jamais levée vers son propre but — rondo.js ; absent : hier */ }, porteeGeste: { talonnade: 12, 'passe-pivot': 22 } /* (314) le geste de dos a sa portée — strike-sim.js ; absent : hier */,   /* (296) LA PORTÉE EST CELLE D'UN PIED (portee.js) : tout coéquipier à ≤ max m est une option du barème, levée au-delà de leve m ; null : la portée de 13 m du rondo */ toucheCorps: null,   /* (292) LA TOUCHE SUIT LE CORPS (touche-corps.js) — RÉFUTÉE à la mesure, éteinte par défaut : opt-in { kMin: 0.5, pow: 1, vCorps: 2, vMin: 2 } — les échappées de conduite baissent d'un tiers, les pertes ne baissent pas (elles se déplacent : la défense collée) */ pokeReach: 0.5, tacleDebout: { cone: 55, k: 0.56, esq: 0.15, battu: 0.22 },   /* (291) LE TACLE DEBOUT (tacle-debout.js, Modèle 11 § 4) : le pied ne balaie que ± cone° devant le buste, réussi à la note × k (calé sur le monde : le ballon touché une fois sur deux, le book), le dribbleur esquive, le battu s'assoit battu s ; null : le pique d'hier */ // m — LE PIQUE : un ballon de conduite libre à portée de pied adverse
  ...
  compensation: null,     // LA CHAISE À QUATRE PIEDS côté attaque (230, compensation.js) — loi NOMMÉE, ÉTEINTE par défaut (null = le monde 229 au bit). Mesuré : nos attaques gardent déjà 6,5 corps derrière le ballon (réel 4-5) ; allumée côté ballon elle comble le couloir du latéral monté (24 → 40 %) mais le milieu descendu devient la sortie arrière de l'ailier (4 buts / 20 × 300 s, débordements −30 %, services aux coureurs morts) ; côté opposé seul, rien de mesurable. Un projet l'allume : { monte: 12, hyst: 3, ext: 10, part: 0.7, bonus: 6, tenue: 1, memoR: 25, pivot: false, transition: false, oppose: true, opposeM: 20 } — le latéral monté ≥ monte m devant sa ligne réelle, le milieu posté le plus proche du spot vacant (rôle press : bonus m) tire son spot vers lui (part × posF), fidèle et tenant sa cadence d'appel (tenue s), jamais en transition, côté opposé au ballon (oppose).
  projection: null,       // L'ENTRE-LIGNES (231, projection.js) — loi NOMMÉE, ÉTEINTE par défaut (null = le monde 229 au bit). Mesurée allumée : les intérieurs vivent devant le ballon (milieu −5,2 → −2,1 m, 0,70 → 0,97 corps devant, entre-lignes milieu → avant 15,6 → 8,6) mais le ballon monte par le centre : appels profonds −30 %, débordements −45 %, passes profondes 13 → 6, et 16 buts / 20 × 300 s (229 : 11). Un projet l'allume : { entre: 8, marge: 10, installe: 3, depuis: 0, part: 1 } — en possession installée (installe s) et ballon à x ≥ depuis, les intérieurs postés (milieu hors pivot, dispensés du comité des soutiens) tirent leur spot (part) vers ballon + entre × axe(hauteurBloc) × axe(rôle profondeur), borné par la ligne de hors-jeu − marge, et ne sont pas candidats à l'accompagnement.
  ...
  calmFoe: 1.8, presseLue: { seuil: 0.55, pressT: 1.5, pose: 1.15, vif: 0.85 }, conduiteSerree: null,   /* (290) LA CONDUITE SE SERRE SOUS PRESSION (serre.js) — RÉFUTÉE à la mesure, éteinte par défaut ; opt-in { p0: 0.3, min: 0.45, kP: 0.9, dv: 0.6 } (ou franc : min 0.25, kP 1.2, dv 0.2) : la touche se serre, les piques ne baissent pas (ils tombent sur des touches prises AVANT que le presseur n'arrive) */   /* (289) LA PRESSION SE LIT AU TEMPS D'ARRIVÉE (presse-lue.js) : pressé si P = 1 − TTP/(pressT × anticipF) ≥ seuil × 1,075/composureF × axe(tempo, pose, vif) — le calme, le retournement et le libre la suivent ; null : le 1,8 m d'hier au bit */
};
