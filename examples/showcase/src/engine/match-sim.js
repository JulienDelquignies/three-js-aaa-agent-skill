...
export { MATCH };
import { huitSecondes } from './temps.js'; import { ligneStep } from './ligne.js'; import { bordFiletStep, onOut, canTake, chronoStep, tempoWait, feuilleDeMatch, administerWhistle, adjugeFaute, remiseEnTouche, coupFrancDirect, coupFrancLance, cornerTrav, cornerSpots, toucheSpots, stepRemplacements, ballFetch, kickoffSpots, placeKickoff, onTakeMatch, arbitreStep, elireTaker, elanJob, elanNow } from './referee.js'; import { tryShot, tryCross, tryClear } from './shooting.js';
...
function assignMatchJobs(st, cfg) {
  ...
  // ---- la défense : press sur le ballon, cover CÔTÉ BUT, marquage goal-side
  {
    ...
    }); if (st.full && cfg.ligne) ligneStep(st, cfg, { defTeam: defTeamB, membres: defenders.filter((q) => (mapD[q.post ?? 9] ?? 9) < nDefD && !q.keeper && q.down <= 0 && !q.expulse && !q._sub), ownX: defGoal.x, sgnDef, couvert: st._bCouvert?.[defTeamB]?.etat ?? null });   // (273) LA LIGNE EST UNE LIGNE (ligne.js) : l'unité relit sa référence à 4 Hz, borne ses cibles à la bande, retient les avancés
  }
