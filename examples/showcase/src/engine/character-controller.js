...
export class CharacterController {
  constructor(model, { mixer, runClip, idleClip, legs = null, stride = 2.6, runSpeed = 5.5, accel = 14, turnRate = 12, forwardLocal = FWD } = {}) {
    ...
    this.stride = stride; this.runSpeed = runSpeed; this.accel = accel; this.turnRate = turnRate;
  ...
  update(dt) {
    ...
    const mag = this._cur.length(); this.speed = mag * this.runSpeed;
    if (mag > 0.02) {
    ...
    const run01 = Math.min(1, this.speed / this.runSpeed);
    ...
    if (this.footLock && run01 > 0.25) this.footLock.solve();
  }
}
