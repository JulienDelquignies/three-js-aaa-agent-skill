...
export function makeTicker(TEAMS) {
  ...
  const event = (e, state) => {
    ...
    if (e.type === 'skill' && hud && !e.kind.endsWith('-vendu')) {
      ...
      pousse(`<b style="color:#e8ebf2">${l}</b> <span>— ${team(state, e.by)} nº${e.by} · ${tFM(e.t)}</span>`);
      ...
      pousse(`<b style="color:#8ecae6">une-deux lancé</b> <span>— nº${e.a} avec nº${e.b} · ${tFM(e.t)}</span>`);
    } else if (e.type === 'celebration' && hud) {
      // LE BUT SE FÊTE (lot 116) : l'événement nomme le buteur et ses compagnons de course
      pousse(`<b style="color:#90be6d">il célèbre !</b> <span>— nº${e.by} et ${e.avec?.length ?? 0} coéquipiers · ${tFM(e.t)}</span>`);
      ...
      pousse(`<b style="color:#f4a261">${noms[e.posture] ?? e.posture}</b> <span>— ${TEAMS[e.team ?? 0].name} · ${tFM(e.t)}</span>`);
      ...
      pousse(`<b style="color:#8ecae6">pressing</b> <span>— ${TEAMS[e.team ?? 0].name} (${(e.kind ?? '').replace(/-/g, ' ')}) · ${tFM(e.t)}</span>`);
    } else if (e.type === 'but') {
      sifflet('but !', '#90be6d');
      ...
      pousse(e.couleur === 'rouge'
        ? `<b style="color:#d62828">carton rouge</b> <span>— nº${e.by} · ${tFM(e.t)}</span>`
        : `<b style="color:#ffd60a">carton jaune</b> <span>— nº${e.by} (${e.cumul}ᵉ) · ${tFM(e.t)}</span>`);
      ...
      pousse(`<b style="color:#f2c14e">hors-jeu</b> <span>— ${team(state, e.by)} nº${e.by} · ${tFM(e.t)}</span>`);
      ...
      pousse(e.type === 'faute'
        ? `<b style="color:#e76f51">faute</b> <span>— nº${e.by} sur nº${e.sur} · ${tFM(e.t)}</span>`
        : `<b style="color:#90be6d">avantage</b> <span>— ${team(state, e.sur)} joue · ${tFM(e.t)}</span>`);
    } else return false;
    return true;
  };
