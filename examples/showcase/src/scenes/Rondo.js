...
export class Rondo {
  ...
  async _build() {
    ...
    const model = this.fullMode
      ...
      : this.matchMode
        ? generateStadium({ tier: 4, landmark: 'grandbol', ...(this.duelMode ? CAGE_STADE : { pitch: { L: 46, W: 30, circle: 4, box: { d: 8, w: 15 }, six: { d: 3, w: 9 }, spot: 7.5 }, goal: { w: 5, h: 2 } }) })
    ...
    this._mcfg = this.fullMode ? matchCfg({ shotRange: 20, chrono: { periodes: 2, duree: 180, pause: 6 } })
      : this.duelMode ? duelCfg() : this.matchMode ? matchCfg() : null;
    this.state = this.duelMode ? makeDuel({ seed: Number(q.get('seed')) || 7 }) : this.matchMode
  ...
  camera(cam, controls) {
    ...
    const back = this.fullMode ? (narrow ? 42 : 47)
      : this.duelMode ? (narrow ? 11 : 12.5) : this.matchMode ? (narrow ? 17 : 20) : 19 - (5 - this.perTeam) * 1.6 - (narrow ? 3.5 : 0);   // (duel) plus près, plus bas : un 1c1 se lit aux appuis
    ...
    cam.fov = this.fullMode ? (narrow ? 60 : 54) : this.duelMode ? (narrow ? 50 : 40) : this.matchMode ? (narrow ? 56 : 50) : (narrow ? 34 : 30); cam.updateProjectionMatrix();
    cam.position.set(0, this.fullMode ? 40 : this.duelMode ? 6.5 : this.matchMode ? 19 : 8.5 - (narrow ? 1.2 : 0), -back);
  ...
  _playTech(pl, e, from = 0) {
    ...
    const r = pl.gestureLayer.begin(useMirror ? (spec.generated && pl.profile ? (pl.moves[`${spec.name}-gauche`] ??= mirrorGen(spec, pl.profile)) : mirrorMove(spec)) : spec);   // (2026-09-26) le geste GÉNÉRÉ se reflète au profil du rig (motion-rig.mirrorGen) : l'Euler local inversé posait le pied gauche derrière le corps sur le Biped
  ...
  update(dt) {
    ...
    for (const pl of this.players) {
      ...
      { const r = stx.restart, o = stx.ball.owner != null ? stx.players[stx.ball.owner] : null, tk = !!r && r.type === 'touche' && r.taker === s.id && Math.hypot(s.p[0] - r.p[0], s.p[2] - r.p[1]) < 1.3, aT = s.act, tId = aT?.payload?.pick?.tech?.id;
        ...
        /* (A12c) LA PAUSA (253, p._pausa) : la semelle sur le ballon RÉEL, en repère personnage */ if ((s._pausa || s._face) && !s.act) { const me = pl.model.matrixWorld.elements, dx = stx.ball.p[0] - s.p[0], dz = stx.ball.p[2] - s.p[2], rl = Math.hypot(me[0], me[2]) || 1, fl = Math.hypot(me[8], me[10]) || 1, at = [(dx * me[0] + dz * me[2]) / rl, (dx * me[8] + dz * me[10]) / fl]; pl.ctrl.idleForce = 'pausa'; pl.ctrl.idleOpts = { override: { raise: Math.hypot(at[0], at[1]) <= 0.55 ? { side: s._face ? (s._face.pied === 'left' ? 'Left' : 'Right') : at[0] < 0 ? 'Left' : 'Right', at: [Math.max(-0.25, Math.min(0.25, at[0])), Math.max(-0.42, Math.min(-0.12, at[1]))], toe: 8, dz: s._face ? 0.1 : 0 } : null } }; } else if (r?.type === 'corner' && r.elan?.phase === 'attend' && r.taker === s.id) { pl.ctrl.idleForce = 'signal'; pl.ctrl.idleOpts = null; } else if (pl.ctrl.idleForce === 'pausa' || pl.ctrl.idleForce === 'signal') { pl.ctrl.idleForce = null; pl.ctrl.idleOpts = null; } pl._holdHands = tk || ((tId === 'touche' || tId === 'roule-main') && !aT.fired) || remiseHands(pl, aT, stx, s) || ((pl.gestureLayer.spec?.name ?? '') === 'ramassage' && stx.ball.owner === s.id); }
      ...
      // clip possède la tête), avant le warp. La politique lit la photo sim que la scène a déjà.
      {
        ...
        const view = {
          ...
          flightTo: st.pass?.to ?? null, justReceivedAt: pl._rxAt ?? null, scan: s.scan ?? null, pos: s.p, /* (A12a) l'horloge de scan de la sim (250), lue — jamais tenue — ici ; (A12f) noLook : le technicien pressé qui donne court */ noLook: !!(a && st.ball.owner === s.id && (s.skill?.gesteF ?? 1) >= 1.05 && targetP && Math.hypot(targetP[0] - s.p[0], targetP[2] - s.p[2]) < 12 && st.players.some((q) => q.team !== s.team && !q.keeper && Math.hypot(q.p[0] - s.p[0], q.p[2] - s.p[2]) < 2.5)),
