  ...
  update(dt) {
    ...
    for (let i = before; i < this.state.events.length; i++) {
      ...
      if (e.type === 'windup') {
        ...
        const pl = this.players[e.by];
        if (pl) { pl._touchT = pl._touchPre != null && this._t >= pl._touchPre - 0.25 && this._t - pl._touchPre < 0.15 ? Math.min(pl._touchPre, this._t) - 0.15 : this._t; pl._touchPre = null;   /* (308) prévue : le sommet au contact ; sinon le geste PART (le saut d'une image du 305 : une téléportation du pied) */ pl._touchFoot = e.foot ?? null; }   // (note 388) le warp de touche suit le PIED que la sim nomme — (304) et commence AVANT le contact quand la touche était prévue (_predictTouch)
