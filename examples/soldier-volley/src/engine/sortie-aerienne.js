...
export function sortieAerienne(st, gk, cfg, pitch) {
  ...
  for (const s of path) {
    ...
    if ((SA.reaction ?? 0.2) + (dR < vM * vM / (2 * a) ? Math.sqrt(2 * dR / a) : vM / (2 * a) + dR / vM) > s.t - (SA.marge ?? 0.1)) continue;   // il n'y serait pas avant le ballon
    let duel = false;
    for (const q of st.players) { if (q.team === gk.team || q.down > 0 || q.expulse || q._sub) continue; if (hyp(s.p[0] - q.p[0], s.p[2] - q.p[2]) / (SA.duel ?? 7) + 0.1 < s.t) { duel = true; break; } }
    if (duel) continue;                                                                // un attaquant y sera avant : le ciel se joue à la tête (tete.js)
    best = { p: s.p, t: s.t, dK }; break;
  }
  ...
  if (best.p[1] >= (SA.sautDes ?? 1.9) && best.t <= mv.contact + 0.05 && best.dK <= (SA.detente ?? 1.0) + 0.2) {   // au-dessus de la prise debout (receive → 'prise-gardien' sous 1,9 m) : le ballon arrive dans le temps de contact du clip, le point à portée de détente
    ...
    startGesture(gk, { id: 'plongeonPrise', duration: mv.duration, contact: mv.contact }, {
      payload: { kind: 'skill', skill: 'plongeon', ownsBody: true, pick: { foot: sideFoot }, lunge: [ux, uz], speed: Math.min(4, best.dK / Math.max(0.15, best.t)),
        cross: { x: best.p[0], z: best.p[2], y: best.p[1], t: best.t }, lungeMax: Math.min(0.72, best.dK + 0.1), aerienne: true },
      log: st.gestures,
    });
    st.events.push({ t: +st.t.toFixed(2), type: 'windup', by: gk.id, move: 'plongeonPrise', foot: sideFoot, skill: 'plongeon', anticipation: mv.contact, mains: 2, sortie: true, h: +best.p[1].toFixed(2), dans: +best.t.toFixed(2) });
  }
  return true;
}
