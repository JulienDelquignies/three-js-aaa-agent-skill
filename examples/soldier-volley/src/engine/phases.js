...
import { estEntreLignes } from './projection.js';
...
export function accompagneMontee(st, cfg, { tac, axe, role }) {
  ...
  const cands = st.players.filter((q) => q.team === c.team && !q.keeper && q.down <= 0 && !q.act
    && q.id !== c.id && !estCompensateur(st, q.team, q.post) && !estEntreLignes(st, q.team, q.post) && (q.p[0] - c.p[0]) * sg > -12 && (q.p[0] - c.p[0]) * sg < (AC.devant ?? 7) + 2
