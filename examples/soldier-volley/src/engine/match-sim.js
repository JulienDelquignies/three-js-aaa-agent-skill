...
function assignMatchJobs(st, cfg) {
  ...
  // ---- la défense : press sur le ballon, cover CÔTÉ BUT, marquage goal-side
  {
    ...
    }); if (st.full && cfg.ligne) { const arrL = defenders.filter((q) => (mapD[q.post ?? 9] ?? 9) < nDefD && !q.keeper && q.down <= 0 && !q.expulse && !q._sub), UL = ligneStep(st, cfg, { defTeam: defTeamB, membres: arrL, ownX: defGoal.x, sgnDef, couvert: st._bCouvert?.[defTeamB]?.etat ?? null }); if (UL && cfg.interligne) { const nMidD = (LIGNES[formationPour(tac(st, defTeamB).formation, false)] ?? [4, 3])[1] ?? 3; interligneStep(st, cfg, { defTeam: defTeamB, milieux: defenders.filter((q) => { const k = mapD[q.post ?? 9] ?? 9; return k >= nDefD && k < nDefD + nMidD && !q.keeper && q.down <= 0 && !q.expulse && !q._sub; }), arriere: arrL, ref: UL.ref, ownX: defGoal.x, sgnDef, hauteur: tac(st, defTeamB).hauteurBloc }); } }   /* (274) L'INTERLIGNE DÉRIVÉ (interligne.js) : le milieu suit la ligne tenue à la portée de ses attributs, la rupture se publie */   // (273) LA LIGNE EST UNE LIGNE (ligne.js) : l'unité relit sa référence à 4 Hz, borne ses cibles à la bande, retient les avancés
  }
