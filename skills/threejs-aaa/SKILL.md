...
| **Generated foot restarts (motion-restart, lot A9 bis): the free-kick and corner RUN-UP (the taker walks back, waits, runs in and the sim takes the restart at the gesture's contact on arrival — yesterday an instant strike from a standing body), the keeper's volley clearance (`voleeGardien`: the gloves carry the ball to the drop point, it falls, the instep takes it at mid-height and the pass leaves from the ball's real height), the thrower behind the touchline, the held aerial catch; the SIM under cfg.remisesPied (absent = yesterday bit for bit); 36 clauses** | [reference/56-remises-au-pied.md](reference/56-remises-au-pied.md) |
