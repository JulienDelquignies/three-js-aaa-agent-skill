...
| **Generated gestures (motion-rig + motion-strike + motion-control + motion-aerial + motion-skill + motion-ground + motion-keeper + motion-cast): 41 species — strikes, controls, headers, dribbling skills, slide tackle, keeper dives and parades — as anatomical joint curves and IK foot paths → animkit specs, rig profile with sign probe, per-player style, the contact sheet; in play the swing runs at ×1 and the contact key lands on the sim's fire tick (`scenes/rondo-fusion.js`, A2)** | [reference/51-motion-strike.md](reference/51-motion-strike.md) |
| **Generated locomotion (motion-gait): walk → jog → run → sprint, backpedal and side shuffle as a PURE FUNCTION of gait phase and body velocity — foot paths fixed to the ground by construction, heel-to-toe roll, flight arc, pelvis/trunk/arms — posed absolutely by the controller after the mixer, direction-aware cadence, per-player gait signature; `?foulee=clips` and `contact-sheet --gait` for the before/after** | [reference/52-motion-gait.md](reference/52-motion-gait.md) · `engine/motion-gait.js` · `scripts/verify-foulee.mjs` |
| **Generated idles (motion-idle): rest, hands on hips, bouncing, defender's guard, keeper's set position, the wall — a PURE FUNCTION of time, kind and style (weight shifting foot to foot, breathing shoulders, feet fixed by IK, humerus twist for posed hands), chosen by a pure policy from the sim situation and the persona; `contact-sheet --idle`** | [reference/53-motion-idle.md](reference/53-motion-idle.md) · `engine/motion-idle.js` · `scripts/verify-attente.mjs` |
| **Generated hand restarts (motion-restart): throw-in, keeper's underarm roll, ground pickup — three generated species + a wrist-to-point arm IK (two-bone, elbow plane from the pole, frame-matrix rotation); the SIM arms the throw-in and the ball leaves the hands at contact (throwNow), the keeper distributes by hand (roll), the thrower waits facing the pitch and turns onto his target** | [reference/54-motion-restart.md](reference/54-motion-restart.md) |
| **Generated contact (motion-contact): three falls with their get-up, the stumble, the shoulder duel, the ball shield — a leg IK whose knee plane comes from a pole that follows the leg; the SIM names the fall (cfg.contact, absent = yesterday bit for bit) and the retreating presser faces the carrier; the scene holds the fallen body as long as the sim does and gets up on the sim's clock** | [reference/55-motion-contact.md](reference/55-motion-contact.md) |
| **Generated foot restarts (motion-restart, lot A9 bis): the free-kick and corner RUN-UP (the taker walks back, waits, runs in and the sim takes the restart at the gesture's contact on arrival — yesterday an instant strike from a standing body), the keeper's volley clearance (`voleeGardien`: the gloves carry the ball to the drop point, it falls, the instep takes it at mid-height and the pass leaves from the ball's real height), the thrower behind the touchline, the held aerial catch; the SIM under cfg.remisesPied (absent = yesterday bit for bit); 36 clauses; A9 ter: the long goal kick and the long throw-in take a run-up too, and the two-man wall jumps when the ball leaves (`engine/elan.js`, `cfg.remisesPied.elan.sortieBut / elan.toucheLongue / mur`)** | [reference/56-remises-au-pied.md](reference/56-remises-au-pied.md) |
| **The forgotten ball (cfg.porteAnticipe): during a pass windup the gliding body reached 7.5 m/s while the carried ball trailed 0.38 m behind — one windup in five was refused at contact, the ball dumped, the body running on without it; the ball is now carried to the post-glide stance point with a tight servo and a refused striker brakes and re-collects (refusals 65 → 20, run-away episodes 35 → 2 per 900 s; passes +19 %, turnovers −13 %, a third of the duels were those dumped balls)** | [reference/57-porte-anticipe.md](reference/57-porte-anticipe.md) |
| **The visible signs of the role (lot A12a): the receiver SCANS during the pass flight — gaze.js reads the sim's deterministic scan clock (p.scan, lot 250) instead of pinning the receiver's eyes on the ball; one clock (the local off-ball scan falls silent when the sim carries one), eyes back on the ball for the take (< 1.5 m), the head follows at 600°/s and is at 0° for the take; flux: 29 % of the flight frames off the ball, 0.89 saccade/s, 42/43 takes with the eyes on the ball; key absent = yesterday's policy, no game bit. A12b the receiver's POSTURE: the sim already opens the body (lot 170) but the receiver never stands still (0 % of flight frames under 0.6 m/s) — a `reception` idle species (ready stance, arms in balance) and a receiver flag on the generated stride (+12° arm elevation, +16° elbow, swing × 0.55), flag absent = yesterday's stride bit for bit. A12c the pausa: a `pausa` idle species with the sole on the REAL ball (the scene feeds the ball's position in character frame through the controller's idleOpts) — measured: the sim stops the carrier but leaves the ball 1.6-2.0 m away in 4 of 11 pausas, so the sign waits on the engine. A12d the jockey's posture in the generated stride (opts.jockey: lower, leaning, wider, arms open, swing halved), the flag re-read from the sim's own jockey condition. A12e-f the walking roles (hands on hips at rest and in the generated walk when far from the ball), the corner taker's raised arm during the run-up wait, the no-look pass (eyes on the point opposite the target in the last third of the windup). THE DEBTS TAKEN BACK (three keyed sim laws, each null = the previous world bit for bit): cfg.pausaPied — the sole blocks the ball at the pausa decision and the ball is carried, never pushed, during the hold (7/7 pausas with the ball at the foot, before 4/11 at 1.6-2 m); cfg.recevoirSurPlace — a pass to feet with nobody within 4 m is waited for instead of met (flights with a waiting instant 14 vs 10 %, passes −11 %, turnovers −6 %); cfg.pasDeRecul — the cross receiver marked within 1.6 m steps 0.8 m away from his marker at the cross. THE ARM CARRIAGE (user feedback): calm arms at reception, the openness comes from `persona.bras`, and a roster can pose persona and stride/idle/strike styles per player (`squads[team][i].persona`, `.style`, docs/Interface_Style_Joueur.md)** | [reference/58-signes-visibles-du-role.md](reference/58-signes-visibles-du-role.md) |
| **The ground, the joy, the low dive (lots A10 bis, A11): the fouled player stays down under `cfg.sol` (chute + tenue × speed, 2.2-2.8 s; he drops the ball and cannot act while down — measured: a downed player striking 0.5 s after his fall; a lying body keeps standing players ≥ 0.9 m away — the tackler stood 0.5 m into him); the held pose LIVES (motion-contact: a closed cycle between `lying` and `rise` — hand to the head, head down/up, top leg folding — the scene ping-pongs it at ×0.6 while the sim holds, then advances at (T−t)/down so the body is up exactly when the sim lifts, no pose jump, proven in pure); the tackle winner's ball take no longer plays the chest `amorti` (49° arms), ground receptions without a named technique are foot controls; THE GENERATED EMOTION family (`motion-emotion`: poing, brasLeves, oreille, calme, glissade on the knees, accolade, applaudir, proteste — seven upper-body gestures the stride keeps running under, one knee slide that owns the legs and is held/raised by the contact clock), chosen by the persona under `cfg.fete` (flair → slide/ear, calm → calm, burstiness → fist), companions hug at ≤ 1.6 m, the conceding side walks head down hands on hips, the fouler protests unless calm; `persona.bras` scales `armElev` for every generated family; the low dive turns the chest to the ground (`rollC` 100, top hand 0.92 → 0.33 m) and a slow ball is taken by WAITING (takeoff delay ≤ 0.6 s) rather than diving at ×0.8; key absent = yesterday bit for bit (fingerprint twin), A/B passes +1 %, turnovers −4 %. A11 bis THE REFEREE HAS A BODY: `motion-arbitre` (whistle to the mouth, card held up with the card mesh in the hand, arm pointing the free kick, advantage with both arms while running), queued by the sim under `cfg.arbitreGestes` (whistle → card → pointing; the body stops and turns), played on the central referee's own gesture layer; A10 quater: the assisted rise — when play is stopped a teammate jogs over and offers a hand (`engine/aide.js`, `cfg.sol.aide`)** | `reference/59-le-sol-la-joie-le-plongeon-bas.md` — verify-contact 34/0, verify-emotion 33/0, verify-arbitre 21/0, verify-motion 206/0, verify-attente 54/0 |
...
| **The engine wirings of the generated gestures (lots B1-B6, B10, C1-C3, § 9, § 6, § 7, § 5, § 8, § 10, 16/09): the foot on the ball at the shot image (scenes/rondo-warp.js — calibration seeded from the generated clip, calibrated at the shot frame, the pelvis lunge before the foot lock: foot→ball-surface gap 4 cm median, was 30 cm ankle→centre), the immediate kick of the far free kick and the possession corner in the elan-contact image (remisesPied.elan.tirImmediat — the plan taken at the pose, the run-up waits for its man, no second gesture), the ARMED header (cfg.tete.armee — the act armed one clip-contact ahead of the predicted ball, the body runs under its windup, the header resolves at the act's contact), the wall at a trot whose body deflects the low free kick (loi12.murTrot, remisesPied.mur.corps — the ground ball passes under the jumping wall, a wall man never controls the kick that hits him), the smooth heading (cfg.viragesLisses — the requested heading slewed at a rate bounded by speed, braking when far from it) and the planted speed (cfg.plantVitesse — v = 0 under a planting act), the keeper's aerial claim (cfg.sortieAerienne — the run to the point where the free ball comes down in his zone, then the two-handed jump catch armed half a second before it), the armed bicycle kick (cfg.retournee — the authored clip finally triggered: the free ball predicted above the head of an attacker with his back to goal, alone in the box, struck at goal at the act's contact), the hand grab (the helper's hand pulls and the fallen player's hand meets it during his getup — two IKs in the scene), the keeper's punch (a generated sortiePoing clip armed when an attacker arrives with the ball, the ball cleared upfield), the ball held back to goal (cfg.bouclier: the carrier pressed in the back without a forward option holds, back to the presser, until a support, a push foul or 2 s), the stride's crossover step in sharp turns, the running arm carriage and the foot lock calibrated on the generated stride (A7 ter, verify-foulee 81/0), the pre-match handshake line, the final-whistle salute and the longer card (cfg.ceremonie, verify-ceremonie 8/0), the assistants' flag gestures and the ball boys (cfg.ramasseurs, verify-assistants 13/0, verify-ramasseurs 5/0), the substitute who jogs in and the fouled player who limps for 25 s after a serious foul, the limp fading over its duration (cfg.entrant, cfg.boiterie, verify-boiterie 4/0, verify-foulee 86/0), the small gestures of the match — the taker's sole on the ball at a goal kick, the keeper pointing his wall, the defensive header, the oriented first touch, the body feint before a run (cfg.petitsGestes, verify-petits-gestes 15/0), the stepovers fed — the jockey window, the square-up, the ball at the clip's spot (cfg.passements, verify-passements 6/0), the chest control chained into a volley or a bicycle kick (cfg.enchainement, verify-enchainement 8/0), the applause on a save and at the final salute (the last orphan of docs/Inventaire_Gestes.md), the named dribbling touches — foot and surface decided by the engine, played by the scene (cfg.conduiteNommee, verify-conduite 9/0). Each key null = yesterday bit for bit (footprint twin); docs/Branchements_Moteur_Animations.md is the working list** | [reference/51-couche-de-geste-et-warp.md](reference/51-couche-de-geste-et-warp.md), [reference/56-remises-au-pied.md](reference/56-remises-au-pied.md), [reference/51-motion-strike.md](reference/51-motion-strike.md), [reference/52-motion-gait.md](reference/52-motion-gait.md), [reference/53-terrain-gardien-match.md](reference/53-terrain-gardien-match.md) The match read-through of 17/09 (notes 395-398): `cfg.orientationPasse` (the pass technique chosen for the turn, the carrier opens before winding up), `cfg.verticalite` (the forward pass and the space ahead with their range, the retreat that pays), `cfg.decalage` (the shoulder attacked, the hook and the stepover on the run) — benches verify-orientation, verify-verticalite, verify-decalage ; its second pass (notes 399-402): `cfg.toucheOrientee` (the free receiver's first touch takes the ball into the open side), `cfg.verticalite.appel`, `cfg.orientationPasse.talon` (the honest heel), `cfg.decalage.sortie` — bench verify-touche-orientee. |
