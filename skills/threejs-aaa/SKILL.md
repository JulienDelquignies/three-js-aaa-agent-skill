...
| **The engine wirings of the generated gestures (lots B1-B6, 16/09): the foot on the ball at the shot image (scenes/rondo-warp.js — calibration seeded from the generated clip, calibrated at the shot frame, the pelvis lunge before the foot lock: foot→ball-surface gap 4 cm median, was 30 cm ankle→centre), the immediate kick of the far free kick and the possession corner in the elan-contact image (remisesPied.elan.tirImmediat — the plan taken at the pose, the run-up waits for its man, no second gesture), the ARMED header (cfg.tete.armee — the act armed one clip-contact ahead of the predicted ball, the body runs under its windup, the header resolves at the act's contact), the wall at a trot whose body deflects the low free kick (loi12.murTrot, remisesPied.mur.corps — the ground ball passes under the jumping wall, a wall man never controls the kick that hits him), the smooth heading (cfg.viragesLisses — the requested heading slewed at a rate bounded by speed, braking when far from it) and the planted speed (cfg.plantVitesse — v = 0 under a planting act). Each key null = yesterday bit for bit (footprint twin); docs/Branchements_Moteur_Animations.md is the working list** | [reference/51-couche-de-geste-et-warp.md](reference/51-couche-de-geste-et-warp.md), [reference/56-remises-au-pied.md](reference/56-remises-au-pied.md), [reference/51-motion-strike.md](reference/51-motion-strike.md), [reference/52-motion-gait.md](reference/52-motion-gait.md) |
