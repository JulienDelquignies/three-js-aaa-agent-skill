...
| **The engine wirings of the generated gestures (lots B1-B3, 16/09): the foot on the ball at the shot image (scenes/rondo-warp.js — calibration seeded from the generated clip, calibrated at the shot frame, the pelvis lunge before the foot lock: foot→ball-surface gap 4 cm median, was 30 cm ankle→centre), the immediate kick of the far free kick and the possession corner in the elan-contact image (remisesPied.elan.tirImmediat — the plan taken at the pose, the run-up waits for its man, no second gesture), the ARMED header (cfg.tete.armee — the act armed one clip-contact ahead of the predicted ball, the body runs under its windup, the header resolves at the act's contact). Each key null = yesterday bit for bit (footprint twin); docs/Branchements_Moteur_Animations.md is the working list** | [reference/51-couche-de-geste-et-warp.md](reference/51-couche-de-geste-et-warp.md), [reference/56-remises-au-pied.md](reference/56-remises-au-pied.md), [reference/51-motion-strike.md](reference/51-motion-strike.md) |
