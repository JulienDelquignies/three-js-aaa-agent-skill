...
{
  const cfg = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null,  shotRange: 20, carton: null });
...
{
  const cfg = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null,  shotRange: 20, carton: null, loi3: { changements: 1 } });
  const st = settle(3, cfg);
  ...
  const st2 = settle(3, matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null,  shotRange: 20, carton: null }));
  const c = st2.players.find((p) => p.team === 1 && !p.keeper);
  for (let k = 0; k < 4; k++) siffle(st2, matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null,  shotRange: 20, carton: null }), c.id);
  ok(`l'EXPULSÉ est irremplaçable (rouge posé : remplacer=${remplacer(st2, matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null,  shotRange: 20, carton: null }), 1, c.id, null)} — l'équipe RESTE à 10)`,
    c.expulse === true && remplacer(st2, matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null,  shotRange: 20, carton: null }), 1, c.id, null) === false);
}

// ---------- 4. sabotage nommé « porte tournante fermée » : loi3 absent → l'API refuse tout
{
  const cfg = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null,  shotRange: 20, carton: null, loi3: false });
  const st = settle(3, cfg);
  const q = st.players.find((p) => p.team === 1 && !p.keeper);
...
// ---------- 5. lot 184 : L'ENTRÉE À LA MÉDIANE (cfg.entreeMediane — la Loi 3 complète)
{
  const entre = (over) => {
    ...
    const cfg = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null,  shotRange: 20, repli: false, ...(over ?? {}) });   // (221) repli:false — la clause dépend de LA première remise après 30 s de la graine 3 ; le repli en change le monde et le changement demandé s'annule à la reprise
