...
{
  const st = makeMatch({ full: true, seed: 3 });
  const cfg = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null,  shotRange: 20, chrono: { periodes: 2, duree: 60, pause: 5 } });
  let avant = null;
...
// ---------- 4. l'ATTRIBUT module : l'endurant tient, même match, même graine
{
  ...
  const stamApres = (note) => {
    ...
    const cfg = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null,  shotRange: 20, allure: false });
...
{
  const st = makeMatch({ full: true, seed: 3 });
  const cfg = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null,  shotRange: 20, fatigue: false });
