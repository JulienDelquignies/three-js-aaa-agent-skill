...
{
  const cfg = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null,  shotRange: 20, carton: null /* carton null DATÉ 257 : ce banc mesure la RÉCIDIVE du 25 (jaune à 2, second jaune rouge) ; le monde 257 juge la nature au flux */ });
...
{
  const cfg = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null,  shotRange: 20, carton: null, loi12: { avantage: 1.8, contact: 0.9, mur: 9.15, jaune: 0 } });
  const st = settle(3, cfg);
  const par = st.players.find((q) => q.team === 1 && !q.keeper).id;
