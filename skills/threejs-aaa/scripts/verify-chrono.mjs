...
const joue = (seed, over = {}) => {
  ...
  const cfg = matchCfg({ ceremonie: null,  shotRange: 20, chrono: CH, ...over });
...
// ---------- 1. le cycle : mi-temps, engagement alterné, sifflet final, état terminal
{
  ...
  for (let i = 0; i < 120; i++) matchStep(st, 1 / 60, matchCfg({ ceremonie: null,  shotRange: 20, chrono: CH }));
...
// ---------- 3. l'ÉCHANGE DE CAMPS (Loi 8) + la MONTRE (lot 24)
{
  ...
  const cfg = matchCfg({ ceremonie: null,  shotRange: 20, chrono: CH });
  ...
  const cfg2 = matchCfg({ ceremonie: null,  shotRange: 20, chrono: { ...CH, echangeCamps: false } });
  ...
  const cfg3 = matchCfg({ ceremonie: null,  shotRange: 20, chrono: { ...CH, additionnel: false } });
  let mt3 = null;
