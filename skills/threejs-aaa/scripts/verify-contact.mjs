...
const B_1609 = { tete: TETE_1609, loi12: LOI12_1609, viragesLisses: null, plantVitesse: null, sortieAerienne: null, retournee: null, bouclier: null };   // les clés des lots B, éteintes : chaque clause de flux mesure le monde de son jour
let pass = 0, fail = 0;
...
// ---- 5. LA SIM (cfg.contact) : le fauté tombe, nommé ; le presseur qui recule fait face ; la clé absente rend l'hier
{
  const run = (over) => {
    ...
    for (const seed of [7, 3, 1, 5, 2, 4]) {   // 3 → 6 graines (le porté qui anticipe — cfg.porteAnticipe — a fait tomber les ballons vendangés et avec eux un tiers des duels : 2 chutes sur 3 graines, un compte, pas une loi)
      ...
      const cfg = matchCfg({ ouverture: null /* ouverture null DATÉ 288 : vert à HEAD~ (worktree d90101b), le presseur qui recule fait face 69 % c. 61 sans dans ce monde (les régimes A7 à 2 + 10 % ; 0 ✗ au 287) — la clause mesure sa loi, pas le receveur qui s'ouvre au ballon */, ...MES_1609 /* DATÉ fusion 16/09 : vert dans son parent 5f8870f, la combinaison (275-280) remange la clause — elle mesure sa loi sur le monde de son parent */, ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null, orientationPasse: null, verticalite: null, decalage: null, toucheOrientee: null,  ...B_1609, remisesPied: RP_1609 /* remisesPied hier DATÉ 16/09 (A9 ter, note 368) */, recevoirSurPlace: null /* recevoirSurPlace null DATÉ 15/09 (dettes A12) : vert à HEAD~ (25/0), la réception sur place remange la presse (face 69 c. 65 sans la clé) — la clause mesure sa loi, pas la mienne */, familiarite: null /* familiarite null DATÉ 254 : vert à HEAD~ (25/0 au 255), les chutes remangées par la familiarité (0 chute pour 3 fautes) — la clause mesure le contact, pas la familiarité */, repertoire: null, arretControle: null /* repertoire et arretControle null DATÉ 279 : vert à HEAD~ (worktree 842c118, 25/0), le fauté qui tombe remangé (2 chutes pour 6 fautes) — la clause mesure sa loi, pas les vitesses du book ni l'arrêt au journal */, visee: null /* visee null DATÉ 277 : vert à HEAD~ (25/0 au 276 fusionné), le fauté qui tombe remangé (1 chute pour 9 fautes) : le point visé change les reprises — la clause mesure sa loi, pas le point visé */, recevoirSurPlace: null /* recevoirSurPlace null DATÉ 15/09 (dettes A12) : vert à HEAD~ (25/0), la réception sur place remange la presse (face 69 c. 65 sans la clé) — la clause mesure sa loi, pas la mienne */, blocPercu: null /* blocPercu null DATÉ 275 : vert à HEAD~ (25/0 au 274), le fauté qui tombe remangé (2 chutes pour 7 fautes) : le bloc qui perçoit change les duels — la clause mesure sa loi, pas le bloc perçu */, ligne: null /* ligne null DATÉ 273 : vert à HEAD~ (25/0 au 272), le fauté qui tombe remangé (2 chutes pour 7 fautes) : la ligne tenue change les duels — la clause mesure sa loi, pas la ligne */, familiarite: null /* familiarite null DATÉ 254 : vert à HEAD~ (25/0 au 255), les chutes remangées par la familiarité (0 chute pour 3 fautes) — la clause mesure le contact, pas la familiarité */, shotRange: 20, chrono: { periodes: 2, duree: 180, pause: 6 }, horsJeu: null /* horsJeu null DATÉ 259 : vert à HEAD~ (25/0 au 257), les chutes remangées par la course qui traverse (1 chute pour 6 fautes) — la clause mesure le contact, pas la Loi 11 */, contre: null /* contre null DATÉ 258b : vert à HEAD~ (25/0 au 258), les chutes (2 sur 6 × 300 s pour 9 fautes) remangées par le corps qui contre — la clause mesure le contact, pas le contre */, ...over });
      const downAt = {};
...
// ---- 5 bis. (A10 bis) LA SIM SOUS cfg.sol : le fauché reste à terre plus longtemps, n'agit plus couché, et personne ne marche dans son corps
{
  const run = (over) => {
    ...
    for (const seed of [7, 3, 1, 5, 2, 4]) {
      const st = makeMatch({ full: true, seed }), cfg = matchCfg({ presseLue: null /* presseLue null DATÉ 289 : vert à HEAD~ (worktree 7bd70e0), le fauché, la marche dans un corps couché dans ce monde (39/0 au 288) — la clause mesure sa loi, pas la pression lue au temps d'arrivée */, layoff: null /* layoff null DATÉ 287 : vert à HEAD~ (worktree 4535d0f), la marche dans un corps couché 11,3 % c. 11,5 sans (0 ✗ au 286) — la clause mesure sa loi, pas la une-touche jugée par son angle */, toucheRapide: null, toucheAuPied: null /* toucheRapide et toucheAuPied null DATÉ 284 : vert à HEAD~ (worktree 695d340), la marche dans un corps couché remangée (3,4 % c. ≤ 3 : les touches au pied changent où l'on tombe) — la clause mesure sa loi, pas la touche rapide ni la touche au pied */, remisePostes: null, rendezVous: null /* remisePostes et rendezVous null DATÉ 281 : vert à HEAD~ (worktree 0f5e292), le sol et le relevé aidé remangés (le fauché à terre p50 2,62 s c. sans 1,62 ; la marche dans un corps couché 5,4 % ; le coéquipier au fauché, la main tendue) — la clause mesure sa loi, pas la passe au rendez-vous ni les postes de la remise */, ...MES_1609 /* DATÉ fusion 16/09 : vert dans son parent 5f8870f, la combinaison (275-280) remange la clause — elle mesure sa loi sur le monde de son parent */, ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null, orientationPasse: null, verticalite: null, decalage: null, toucheOrientee: null,  ...B_1609, familiarite: null, remisesPied: { elan: { recul: 3.5, lat: 1.5, vitesse: 4.0, patience: 4 }, volee: { h: 1.0, avance: 0.45, lacher: 0.72 }, touche: { recul: 0.25 } } /* remisesPied A9 bis DATÉ 16/09 (lot A9 ter, note 368 : la sortie de but longue et la touche longue courent, le mur saute — le flux des fautes change ; la clause mesure cfg.sol, pas les remises) */, ...over });
...
// ---- 5 ter. (A10 quater) LE RELEVÉ AIDÉ (cfg.sol.aide) : le jeu arrêté, un coéquipier vient tendre la main au fauché qui se relève
{
  ...
  for (const seed of [3, 7]) {
    const cfg = matchCfg({ presseLue: null /* presseLue null DATÉ 289 : vert à HEAD~ (worktree 7bd70e0), le relevé aidé et la main tendue dans ce monde (39/0 au 288) — la clause mesure sa loi, pas la pression lue au temps d'arrivée */, remisePostes: null, rendezVous: null /* remisePostes et rendezVous null DATÉ 281 : vert à HEAD~ (worktree 0f5e292), le sol et le relevé aidé remangés (le fauché à terre p50 2,62 s c. sans 1,62 ; la marche dans un corps couché 5,4 % ; le coéquipier au fauché, la main tendue) — la clause mesure sa loi, pas la passe au rendez-vous ni les postes de la remise */, ...MES_1609 /* DATÉ fusion 16/09 : vert dans son parent 5f8870f, la combinaison (275-280) remange la clause — elle mesure sa loi sur le monde de son parent */, ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null, orientationPasse: null, verticalite: null, decalage: null, toucheOrientee: null,  ...B_1609, familiarite: null });
  ...
  const A = matchCfg({ presseLue: null /* presseLue null DATÉ 289 : vert à HEAD~ (worktree 7bd70e0), le poste de l'aidant hors du corps couché dans ce monde (39/0 au 288) — la clause mesure sa loi, pas la pression lue au temps d'arrivée */, remisePostes: null, rendezVous: null /* remisePostes et rendezVous null DATÉ 281 : vert à HEAD~ (worktree 0f5e292), le sol et le relevé aidé remangés (le fauché à terre p50 2,62 s c. sans 1,62 ; la marche dans un corps couché 5,4 % ; le coéquipier au fauché, la main tendue) — la clause mesure sa loi, pas la passe au rendez-vous ni les postes de la remise */, ...MES_1609 /* DATÉ fusion 16/09 : vert dans son parent 5f8870f, la combinaison (275-280) remange la clause — elle mesure sa loi sur le monde de son parent */, ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null, orientationPasse: null, verticalite: null, decalage: null, toucheOrientee: null }).sol.aide;
  ...
  ok(runs.every((r) => r.dMin >= matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null, orientationPasse: null, verticalite: null, decalage: null, toucheOrientee: null }).sol.corps - 0.2 && r.jobs.includes('walk')), `…posté hors du corps couché (au plus près ${runs.map((r) => r.dMin.toFixed(2)).join('/')} m ≥ corps ${matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null, orientationPasse: null, verticalite: null, decalage: null, toucheOrientee: null }).sol.corps} − 0,2), arrivé en marchant (métiers ${runs.map((r) => r.jobs.join('→')).join(' ; ')})`);
  {
    const cfg0 = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null, orientationPasse: null, verticalite: null, decalage: null, toucheOrientee: null,  ...B_1609, familiarite: null, sol: { ...matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null, orientationPasse: null, verticalite: null, decalage: null, toucheOrientee: null }).sol, aide: null } });
    ...
    const cfg1 = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null, orientationPasse: null, verticalite: null, decalage: null, toucheOrientee: null,  ...B_1609, familiarite: null });
