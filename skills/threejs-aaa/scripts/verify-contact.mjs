...
// ---- 5 bis. (A10 bis) LA SIM SOUS cfg.sol : le fauché reste à terre plus longtemps, n'agit plus couché, et personne ne marche dans son corps
{
  const run = (over) => {
    ...
    for (const seed of [7, 3, 1, 5, 2, 4]) {
      const st = makeMatch({ full: true, seed }), cfg = matchCfg({ ...MES_1609 /* DATÉ fusion 16/09 : vert dans son parent 5f8870f, la combinaison (275-280) remange la clause — elle mesure sa loi sur le monde de son parent */, ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null,  ...B_1609, familiarite: null, remisesPied: { elan: { recul: 3.5, lat: 1.5, vitesse: 4.0, patience: 4 }, volee: { h: 1.0, avance: 0.45, lacher: 0.72 }, touche: { recul: 0.25 } } /* remisesPied A9 bis DATÉ 16/09 (lot A9 ter, note 368 : la sortie de but longue et la touche longue courent, le mur saute — le flux des fautes change ; la clause mesure cfg.sol, pas les remises) */, ...over });
...
// ---- 5 ter. (A10 quater) LE RELEVÉ AIDÉ (cfg.sol.aide) : le jeu arrêté, un coéquipier vient tendre la main au fauché qui se relève
{
  ...
  for (const seed of [3, 7]) {
    const cfg = matchCfg({ ...MES_1609 /* DATÉ fusion 16/09 : vert dans son parent 5f8870f, la combinaison (275-280) remange la clause — elle mesure sa loi sur le monde de son parent */, ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null,  ...B_1609, familiarite: null });
  ...
  const A = matchCfg({ ...MES_1609 /* DATÉ fusion 16/09 : vert dans son parent 5f8870f, la combinaison (275-280) remange la clause — elle mesure sa loi sur le monde de son parent */, ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null }).sol.aide;
