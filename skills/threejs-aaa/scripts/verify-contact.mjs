...
// ---- 5. LA SIM (cfg.contact) : le fauté tombe, nommé ; le presseur qui recule fait face ; la clé absente rend l'hier
{
  ...
  ok(on.downs.length >= 3 && med(on.downs) >= 1.3 && Math.max(...on.downs) <= 3.2, `le fauché reste à terre p50 ${med(on.downs).toFixed(2)} s (≥ 1,3 : le temps de tomber, de tenir, de se relever ; cfg.contact.chute ${matchCfg({ ...MES_1609 /* DATÉ fusion 16/09 : vert dans son parent 5f8870f, la combinaison (275-280) remange la clause — elle mesure sa loi sur le monde de son parent */, ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null }).contact.chute})`);
...
// ---- 5 bis. (A10 bis) LA SIM SOUS cfg.sol : le fauché reste à terre plus longtemps, n'agit plus couché, et personne ne marche dans son corps
{
  const run = (over) => {
    ...
    for (const seed of [7, 3, 1, 5, 2, 4]) {
      const st = makeMatch({ full: true, seed }), cfg = matchCfg({ remisePostes: null, rendezVous: null /* remisePostes et rendezVous null DATÉ 281 : vert à HEAD~ (worktree 0f5e292), le sol et le relevé aidé remangés (le fauché à terre p50 2,62 s c. sans 1,62 ; la marche dans un corps couché 5,4 % ; le coéquipier au fauché, la main tendue) — la clause mesure sa loi, pas la passe au rendez-vous ni les postes de la remise */, ...MES_1609 /* DATÉ fusion 16/09 : vert dans son parent 5f8870f, la combinaison (275-280) remange la clause — elle mesure sa loi sur le monde de son parent */, ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null,  ...B_1609, familiarite: null, remisesPied: { elan: { recul: 3.5, lat: 1.5, vitesse: 4.0, patience: 4 }, volee: { h: 1.0, avance: 0.45, lacher: 0.72 }, touche: { recul: 0.25 } } /* remisesPied A9 bis DATÉ 16/09 (lot A9 ter, note 368 : la sortie de but longue et la touche longue courent, le mur saute — le flux des fautes change ; la clause mesure cfg.sol, pas les remises) */, ...over });
...
// ---- 5 ter. (A10 quater) LE RELEVÉ AIDÉ (cfg.sol.aide) : le jeu arrêté, un coéquipier vient tendre la main au fauché qui se relève
{
  ...
  for (const seed of [3, 7]) {
    const cfg = matchCfg({ remisePostes: null, rendezVous: null /* remisePostes et rendezVous null DATÉ 281 : vert à HEAD~ (worktree 0f5e292), le sol et le relevé aidé remangés (le fauché à terre p50 2,62 s c. sans 1,62 ; la marche dans un corps couché 5,4 % ; le coéquipier au fauché, la main tendue) — la clause mesure sa loi, pas la passe au rendez-vous ni les postes de la remise */, ...MES_1609 /* DATÉ fusion 16/09 : vert dans son parent 5f8870f, la combinaison (275-280) remange la clause — elle mesure sa loi sur le monde de son parent */, ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null,  ...B_1609, familiarite: null });
  ...
  const A = matchCfg({ remisePostes: null, rendezVous: null /* remisePostes et rendezVous null DATÉ 281 : vert à HEAD~ (worktree 0f5e292), le sol et le relevé aidé remangés (le fauché à terre p50 2,62 s c. sans 1,62 ; la marche dans un corps couché 5,4 % ; le coéquipier au fauché, la main tendue) — la clause mesure sa loi, pas la passe au rendez-vous ni les postes de la remise */, ...MES_1609 /* DATÉ fusion 16/09 : vert dans son parent 5f8870f, la combinaison (275-280) remange la clause — elle mesure sa loi sur le monde de son parent */, ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null }).sol.aide;
  ...
  ok(runs.every((r) => r.dMin >= matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null }).sol.corps - 0.2 && r.jobs.includes('walk')), `…posté hors du corps couché (au plus près ${runs.map((r) => r.dMin.toFixed(2)).join('/')} m ≥ corps ${matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null }).sol.corps} − 0,2), arrivé en marchant (métiers ${runs.map((r) => r.jobs.join('→')).join(' ; ')})`);
  {
    const cfg0 = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null,  ...B_1609, familiarite: null, sol: { ...matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null }).sol, aide: null } });
    ...
    const cfg1 = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null,  ...B_1609, familiarite: null });
