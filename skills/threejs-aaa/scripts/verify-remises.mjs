...
// 'relance-main' n'est jamais tombé en 7 graines × 240 s : la clause force la distribution via beginPass).
{
  ...
  {
    const cfg0 = matchCfg({ ...MES_1609 /* DATÉ fusion 16/09 : vert dans son parent 5f8870f, la combinaison (275-280) remange la clause — elle mesure sa loi sur le monde de son parent */, ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null,  ligne: null /* ligne null DATÉ 273 : vert à HEAD~ (36/0 au 272), les remises sans clé remangées (2 prises sans armé) : la ligne tenue change les corps — la clause mesure sa loi, pas la ligne */, locomoteur: null /* locomoteur null DATÉ 260 : vert à HEAD~ (au 254), la course d'élan, la prise et le lanceur remangés par le profil locomoteur (les corps démarrent en 2,3 τ) — la clause mesure la remise, pas la locomotion */, familiarite: null /* familiarite null DATÉ 254 : vert à HEAD~ (36/0 au 255), le lanceur remangé par la familiarité (bassin −0,72 m) — la clause mesure la touche, pas la familiarité */, horsJeu: null /* horsJeu null DATÉ 259 : vert à HEAD~ (36/0 en worktree 3a78940), le lanceur remangé par l'appel de l'épaule (bassin −0,34 m, face 16°) — la clause mesure la touche, pas la Loi 11 */, shotRange: 20, chrono: { periodes: 2, duree: 180, pause: 6 }, remisesMain: null });
...
// ---- 7. LES REMISES AU PIED (lot A9 bis, cfg.remisesPied)
{
  ...
  {
    const mesure = (over) => {
      const cfgC = matchCfg({ ...MES_1609 /* DATÉ fusion 16/09 : vert dans son parent 5f8870f, la combinaison (275-280) remange la clause — elle mesure sa loi sur le monde de son parent */, ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null,  ...over }); let { st } = playMatch(makeMatch({ full: true, seed: 7 }), 8, { cfg: cfgC });
    ...
    const RPh = matchCfg({ ...MES_1609 /* DATÉ fusion 16/09 : vert dans son parent 5f8870f, la combinaison (275-280) remange la clause — elle mesure sa loi sur le monde de son parent */, ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null, }).remisesPied;
...
{
  const cfg = matchCfg({ ...MES_1609 /* DATÉ fusion 16/09 : vert dans son parent 5f8870f, la combinaison (275-280) remange la clause — elle mesure sa loi sur le monde de son parent */, ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null, });
  const hyp = Math.hypot;
  ...
  // dévié (dévié-mur : il repart vers le tireur, ralenti, relevé), un ballon haut passe ; corps null : le mur traversé d'hier.
  {
    const murTest = (over, { elevation = 0.12, secs = 50 } = {}) => {
      const cfgM = matchCfg({ ...MES_1609 /* DATÉ fusion 16/09 : vert dans son parent 5f8870f, la combinaison (275-280) remange la clause — elle mesure sa loi sur le monde de son parent */, sortieAerienne: null, retournee: null, bouclier: null, ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null, /* DATÉ 15/09 (B4) : les clés du 16/09 changent les 50 s de flux qui posent le coup franc ; le ramasseur retarde la remise, le mur arrive à son point au pas comme au trot */ ...over }); let { st } = playMatch(makeMatch({ full: true, seed: 7 }), secs, { cfg: cfgM });
