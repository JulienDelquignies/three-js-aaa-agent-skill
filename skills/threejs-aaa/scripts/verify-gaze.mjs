...
console.log('\n— A12a : le scan du receveur en vol (p.scan de la sim, lue par la politique — interface gelée §1) —');
{
  ...
  const film = (over) => {
    const st = makeMatch({ full: true, seed: 3 }); const cfg = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null,  ...over });
    const gst = {}, rngs = {};
