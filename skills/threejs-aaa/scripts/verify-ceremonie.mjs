...
const joue = (over, secs, seed = 3) => {
  const st = makeMatch({ full: true, seed }); const cfg = matchCfg({ chrono: { periodes: 1, duree: 40, pause: 4 }, temps: { ...T0, additionnel: { ...T0.additionnel, min: 0 } }, ...over });
...
{
  const h = joue({ ceremonie: null }, 50);
