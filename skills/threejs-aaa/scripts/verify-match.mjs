...
const PRE114 = { skill: { ...matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null }).skill, doubleFoe: null, pontFoe: null, rouletteFoe: null } };
let pass = 0, fail = 0;
...
// ---------- 3. le match joué (les bandes du réel, mesurées sur graines)
{
  ...
  for (const seed of SEEDS) {
    ...
    const { st: s2, trace } = playMatch(st, 120, { cfg: matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null,  ...PRE114 }) });
...
// touches à 1-10° du cap. Ces clauses tiennent LES DEUX : la précision ET la présence.
{
    ...
    regains.length === 0 || regP90 <= 1.15);
  {
    ...
    const tempsLoin = (overrides) => {
      ...
      for (const seed of [3, 7]) {
        const st = makeMatch({ perTeam: 5, seed });
        const cfg = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null,  ...PRE114, ...overrides });
        for (let i = 0; i < 120 * 60; i++) {
          matchStep(st, 1 / 60, cfg);
          const c = st.players[st.possession.carrier];
...
// et LA REMISE NE MIME PAS UN PASSEMENT (le ballon porté roule DEVANT les pieds)
{
  ...
  // chaque foulée : l'œil lisait des passements de jambes sur le retour d'engagement)
  {
    ...
    const cfg = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null,  cadence: null /* cadence null DATÉ 263 : vert à HEAD~ (worktree fe85ce1), le cerveau de champ au tick de 0,1 s — la clause mesure le porté de remise à l'image, pas le pas de décision */,  cadence: null /* cadence null DATÉ 263 : vert à HEAD~ (worktree fe85ce1), le cerveau de champ au tick de 0,1 s — la clause mesure le porté de remise à l'image, pas le pas de décision */ });
    let ahead = [], carried = 0;
    for (let i = 0; i < 120 * 60; i++) {
      matchStep(st, 1 / 60, cfg);
      const r = st.restart;
...
// ---------- 4. les sabotages
{
  ...
  const sansTir = [3, 11].some((seed) => {
    const st = makeMatch({ perTeam: 5, seed });
    const cfg = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null,  tryShot: null, tryCross: null });
...
// ---------- 5. lot 54 — le spin orphelin, le backspin des passes levées, le quart-de-touche nommé
{
  ...
  const quarts = (full, seed, dur) => {
    ...
    const cfg = full ? matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null,  shotRange: 20 }) : matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null });
  ...
  const quartFixture = () => {
    const st = makeMatch({ full: true, seed: 3 });
    const cfg = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null,  uneTouche: false });
