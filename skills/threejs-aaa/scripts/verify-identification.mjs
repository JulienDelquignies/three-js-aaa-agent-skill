...
const EXPRIMES_249 = ['GK A|garde', 'LAT A|profondeur', 'LAT A|appel', 'GK B|garde', 'LAT B|largeurR', 'DC C|profondeur', 'LAT C|profondeur', 'LAT D|largeurR', 'LAT D|profondeur', 'MO A|press', 'MO C|tenue', 'AIL A|largeurR', 'AV A|largeurR', 'AV A|profondeur', 'AV A|repli', 'AV B|repli', 'AIL C|largeurR', 'AV C|repli'];   // REGELÉ DATÉ 265 (le monde de la réception à quatre issues : la touche propre protégée change les possessions, la liste bouge sans qu'aucune loi de rôle n'ait changé) : 23 → 18 signatures — perdues DC A|largeurR, DC A|profondeur, DC A|press, DC B|profondeur, DC C|largeurR, LAT C|appel, MIL A|tenue, MO B|profondeur, AIL B|appel, AIL C|repli, AIL D|largeurR ; gagnées LAT A|appel, LAT D|largeurR, MO C|tenue, AIL A|largeurR, AV A|largeurR, AIL C|largeurR (la dette de volumétrie à 6 graines demeure)
