...
const EXPRIMES_249 = ['GK A|garde', 'DC A|largeurR', 'DC A|profondeur', 'DC A|press', 'LAT A|profondeur', 'LAT B|largeurR', 'LAT C|appel', 'LAT D|largeurR', 'LAT D|profondeur', 'MO A|press', 'MIL B|tenue', 'MO B|profondeur', 'AV A|repli', 'AV B|repli', 'AIL C|largeurR', 'AV C|repli', 'AV C|tenue'];   // REGELÉ DATÉ 263 (le monde du pas de décision — le cerveau de champ au tick de 0,1 s, le gardien et les remises au pas physique : les signatures des défenseurs gagnées au 262 par le marqueur qui suit sa croyance se re-brouillent, la profondeur et le press des centraux s'expriment) : 22 → 17 signatures — perdues LAT A|largeurR, LAT A|appel, GK B|garde, DC B|largeurR, DC B|appel, DC C|largeurR, DC C|appel, LAT C|profondeur, MO A|tenue, MDC C|largeurR, AV A|profondeur, AIL C|repli ; gagnées DC A|profondeur, DC A|press, LAT C|appel, LAT D|largeurR, MIL B|tenue, MO B|profondeur, AV C|tenue ; la volumétrie à 6 graines reste la dette. // REGELÉ DATÉ 262 (le monde de la couche de croyance — le marqueur suit sa croyance de son homme, le passeur vise la sienne : les axes de largeur, d'appel et de profondeur des défenseurs s'expriment) : 15 → 22 signatures — perdues GK A|tenue, DC B|profondeur, LAT D|largeurR, MDC A|largeurR, MDC A|profondeur, MO B|profondeur, AV A|largeurR ; gagnées DC A|largeurR, LAT A|largeurR, LAT A|profondeur, LAT A|appel, DC B|largeurR, DC B|appel, DC C|largeurR, DC C|appel, LAT C|profondeur, MO A|tenue, MDC C|largeurR, AV A|profondeur, AIL C|largeurR, AIL C|repli (la dette de volumétrie à 6 graines demeure)
