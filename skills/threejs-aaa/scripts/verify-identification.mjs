...
const EXPRIMES_249 = ['GK A|garde', 'DC A|largeurR', 'DC A|profondeur', 'GK B|garde', 'DC B|profondeur', 'LAT B|largeurR', 'LAT C|appel', 'LAT D|largeurR', 'LAT D|profondeur', 'MDC A|largeurR', 'MO A|press', 'AV A|repli', 'AV B|repli', 'AV C|repli', 'LAT A|profondeur', 'LAT C|profondeur', 'AV A|profondeur'];   // REGELÉ DATÉ 258 (le monde de l'échelle de finition) : perdues sous ses tirages LAT A|appel, MDC A|profondeur, MDC B|profondeur, MIL B|tenue, MDC C|profondeur, AIL C|repli, AV C|tenue — et AIL D|repli, perdue au 256 (wide_creator sans repli 0,9, l'annexe non lue à ce sceau) ; gagnées LAT A|profondeur, LAT C|profondeur, AV A|profondeur — 17 signatures ; la dette : à 6 graines le test tient au tirage (8 pertes pour un lot qui ne touche pas aux rôles), il lui faut la volumétrie (≥ 12) ; REGELÉ DATÉ 252 (le monde de la passation) : perdues au bord (4/6) DC A|tenue, DC B|tenue, DC C|profondeur, MDC B|press, MDC C|largeurR, AIL A|largeurR, AIL C|largeurR ; gagnées 11 — la règle : à chaque monde scellé la liste se regèle et le journal nomme les pertes
