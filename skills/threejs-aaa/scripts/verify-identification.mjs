...
const EXPRIMES_249 = ['GK A|garde', 'GK B|garde', 'AV A|repli', 'AV B|repli', 'AV C|repli', 'MO A|press', 'AIL C|press', 'DC A|largeurR', 'LAT B|repli', 'MIL A|tenue', 'MO B|profondeur'];   // REGELÉ DATÉ 260 (le monde du profil locomoteur) : perdues LAT B|largeurR, MDC C|largeurR, LAT C|profondeur, DC B|largeurR, DC B|profondeur, DC C|largeurR, DC C|profondeur, LAT D|largeurR, MO A|tenue, MIL B|tenue, MO B|appel, MO C|tenue, AIL A|largeurR, AV A|largeurR, AV A|profondeur, AIL B|appel ; gagnées DC A|largeurR, LAT B|repli, MIL A|tenue, MO B|profondeur (23 → 11 signatures — la dette de volumétrie à 6 graines demeure)
