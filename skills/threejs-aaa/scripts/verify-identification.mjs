...
const EXPRIMES_249 = ['GK A|tenue', 'GK A|garde', 'GK B|garde', 'DC B|profondeur', 'LAT B|largeurR', 'LAT D|largeurR', 'LAT D|profondeur', 'MDC A|largeurR', 'MDC A|profondeur', 'MO A|press', 'MO B|profondeur', 'AV A|largeurR', 'AV A|repli', 'AV B|repli', 'AV C|repli'];   // REGELÉ DATÉ 261 (le monde de l'intention d'effort — le suiveur qui marche tient son slot, les axes de largeur et de profondeur s'expriment à nouveau) : 11 → 15 signatures — perdues AIL C|press, DC A|largeurR, LAT B|repli, MIL A|tenue ; gagnées GK A|tenue, DC B|profondeur, LAT B|largeurR, LAT D|largeurR, LAT D|profondeur, MDC A|largeurR, MDC A|profondeur, AV A|largeurR (la dette de volumétrie à 6 graines demeure)
