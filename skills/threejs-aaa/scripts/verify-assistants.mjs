...
const joue = (over, f) => { const st = makeMatch({ full: true, seed: 3 }); const cfg = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, ...over }); for (let i = 0; i < 60; i++) matchStep(st, 1 / 60, cfg); return f(st, cfg); };
