...
const match = (over, seeds = [3, 7, 11, 15], secs = 300) => {   // 4 graines : à 2 la variance des pertes noyait la clause du monde
  ...
  for (const seed of seeds) {
    const st = makeMatch({ full: true, seed }), cfg = matchCfg({ layoff: null /* layoff null DATÉ 287 : vert à HEAD~ (worktree 4535d0f), les passes longues qui avancent 32 c. 30 hier et 7 tirs c. 8 dans ce monde (0 ✗ au 286) — la clause mesure sa loi, pas la une-touche jugée par son angle */, ...PINS, ...over });
