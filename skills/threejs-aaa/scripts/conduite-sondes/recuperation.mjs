...
// technique nommée. (2026-09-26) seule compte la FORCE sans pied (l'accélération que le roulement n'explique pas) — un ballon possédé qui roule n'est pas un aimant. Usage : node recuperation.mjs <url> [secondes=120] [graines=1,2,3,4] [sortie.json]
...
for (const seed of GR.split(',').map(Number)) {
  ...
  const r = await pg.evaluate((SECS) => {
    ...
    for (let i = 0; i < SECS * 60; i++) {
      ...
      // LA FORCE SANS PIED, pas le roulé : le ballon possédé peut ROULER (cfg.armePied — l'armé ne le tient plus) ; seule compte l'accélération
      // au sol que le roulement n'explique pas (> 2 m/s² d'écart à la décélération du sol, dec0 + decV·v) sans pied à 0,2 m — le servo
      const sol = st.ball.sol, bvN = [st.ball.v[0], st.ball.v[2]], bvP = st.__bvP ?? bvN; st.__bvP = bvN;
      const spP = Math.hypot(...bvP), rou = spP > 0.1 ? (sol ? sol.dec0 + sol.decV * spP : 1.5) : 0, ux = spP > 0.1 ? bvP[0] / spP : 0, uz = spP > 0.1 ? bvP[1] / spP : 0;
      const force = st.ball.p[1] < 0.16 && Math.hypot((bvN[0] - bvP[0]) / dt + ux * rou, (bvN[1] - bvP[1]) / dt + uz * rou) > 2;
      for (const o of ouvertes) if (!o.fini) { const s = sc.players[o.j].sim, d = pied(o.j); o.piedMin = Math.min(o.piedMin, d);
        if (st.ball.owner === s.id) { o.servo++; if (d > 0.2 && force) { o.servoSansPied++; o.glisse += Math.hypot(st.ball.p[0] - o.prev[0], st.ball.p[2] - o.prev[2]); const k = [s.intent ? 'intent' : '', s.anchorHint && st.t - s.anchorHint.t < 0.4 ? 'ancre' : '', s.act ? 'acte:' + (s.act.payload?.skill ?? s.act.payload?.kind ?? s.act.kind ?? '?') : '', (s._prepShot ?? -1) > st.t ? 'prepTir' : '', st._settling && st.t < st._settling.at ? 'pose' : '', s._bouclier ? 'bouclier' : '', s._pausa ? 'pausa' : '', Math.hypot(...s.v) < 1 ? 'lent' : ''].filter(Boolean).join('+') || '?'; (o.pourquoi ??= {})[k] = (o.pourquoi[k] ?? 0) + 1; } }
...
console.log(`  0,6 s APRÈS : ballon possédé ${m((x) => x.servo / 60)} s dont POUSSÉ SANS PIED à 0,2 m (accélération que le roulement n'explique pas) ${m((x) => x.servoSansPied / 60)} s, glissé sous cette force ${m((x) => x.glisse)} m (${P.filter((x) => x.glisse > 0.1).length}/${P.length} > 0,1 m) ; pied au plus près ${m((x) => x.piedMin)} m`);
