...
const rows = await pg.evaluate((SECS) => { const sc = window.__scene, st = sc.state, V = sc.players[0].model.position.constructor, out = []; let ne = 0;
  ...
  for (let i = 0; i < SECS * 60; i++) {
    const snap = st.players.map((s) => ({ pas: s._pas ? { ...s._pas } : null, p: [...s.p], yaw: s.yaw, legK: s.legK, vise: s._pas?.vise ?? null })), ball0 = [...st.ball.p];
    sc.update(1 / 60);
    while (ne < st.events.length) { const e = st.events[ne++]; if (e.type !== 'touche') continue;
      ...
      out.push({ viseAvant: snap[e.by].vise, mode: e.pas ?? '-', lacet: Math.abs(s._pas?.yawRate ?? 0), t: st.t, foot: e.foot, pas: { ...s._pas }, legK: s.legK, p: [...s.p], yaw: s.yaw, ballNow: rel(st.ball.p[0], st.ball.p[2]), ballAvant: rel(ball0[0], ball0[2]), before: snap[e.by], R, steps: sc._simSteps ?? null }); }
  }
  return out; }, +(process.env.SECS ?? 90));
await b.close();
...
const by = {}; for (const r of rows) { if (!r.foot) continue; const f = r.foot === 'left' ? 'Left' : 'Right', d = segD(r.ballAvant, r.R[f + 'Foot'], r.R[f + 'ToeBase']); (by[r.mode] ??= []).push(d); (by[(r.lacet >= 1 ? 'VIRAGE ' : 'droit ') + r.mode] ??= []).push(d); (by['TOUTES'] ??= []).push(d); if (r.mode === 'fin' || r.mode === 'lent') (by[`${r.mode} — pied ${r.viseAvant == null ? 'non visé' : r.viseAvant === r.foot ? 'VISÉ' : 'autre que le visé'}`] ??= []).push(d); }
