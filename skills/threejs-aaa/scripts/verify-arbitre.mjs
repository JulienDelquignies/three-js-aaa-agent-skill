...
{
  const run = (over, f) => { const st = makeMatch({ full: true, seed: 3 }), cfg = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null, orientationPasse: null, verticalite: null, decalage: null, toucheOrientee: null, /* DATÉ 16/09 : la cérémonie d'engagement occupe les 3 s du banc, le ramasseur retarde la remise */ ...over }); for (let i = 0; i < 3 * 60; i++) matchStep(st, 1 / 60, cfg); return f(st, cfg); };
