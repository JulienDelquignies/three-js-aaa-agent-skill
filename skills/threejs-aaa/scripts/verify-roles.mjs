...
// ---------- lot 200 : LE RÔLE AGIT SUR LA STRUCTURE (demande projet aval)
{
  ...
  const slotterEst = (ancA, ancB) => {
    ...
    const cfg = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null, orientationPasse: null, verticalite: null, decalage: null, receveurOuvert: null,  shotRange: 20, soutienN: 1 });
...
// ---- lot 248 : LES 25 RÔLES DU DOCUMENT CONTRE LE CATALOGUE (Campagne V — données seules, empreinte au bit)
{
  ...
  const debordes = (refus) => { let n = 0; for (const seed of [1, 2]) { const sq = Array.from({ length: 11 }, (_, i) => (i === 0 || i === 3) && refus ? { refus: ['deborde'] } : {}); const st = makeMatch({ full: true, seed, squads: [sq, []] }), cfg = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null, orientationPasse: null, verticalite: null, decalage: null, receveurOuvert: null,  shotRange: 20 }); for (let i = 0; i < 300 * 60; i++) matchStep(st, 1 / 60, cfg); for (const e of st.events) if (e.type === 'burst' && e.kind === 'deborde' && st.players[e.by]?.team === 0) n++; } return n; };
...
{
  const bursts = (roleId) => { let n = 0, ahead = 0, img = 0; for (const seed of [1, 2]) { const st = makeMatch({ full: true, seed, roles: [{ 7: roleId }, {}] }), cfg = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null, orientationPasse: null, verticalite: null, decalage: null, receveurOuvert: null,  shotRange: 20 }); for (let i = 0; i < 300 * 60; i++) { matchStep(st, 1 / 60, cfg); if (i % 30 === 0 && st.possession.team === 1 && !st.restart) { const p = st.players.find((q) => q.team === 0 && q.post === 7), sg = Math.sign(st.pitch.ownGoal(0).x || 1); img++; if ((p.p[0] - st.ball.p[0]) * sg < -2) ahead++; if (p._pace?.kind === 'repli' && p._pace.until > st.t) n++; } } } return { n, devant: 100 * ahead / Math.max(1, img) }; };
  const marchant = bursts('wide_creator'), presseur = bursts('tracking_winger'), poly = bursts('polyvalent');
  ok(`lot 251 — LE REPLI PAR RÔLE : l'ailier marchant (wide_creator, interdit 'repli') ne rentre jamais (${marchant.n} images en sprint de repli = 0 sur 2 × 300 s, devant le ballon ${marchant.devant.toFixed(0)} % des images sans ballon), l'ailier de pressing (tracking_winger, repli 0,1) rentre (${presseur.n} ≥ 1, devant ${presseur.devant.toFixed(0)} % — informatif) ; polyvalent (0,5 = l'élection d'hier) ${poly.n} sprints, devant ${poly.devant.toFixed(0)} % ; clé repli.role ${matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null, orientationPasse: null, verticalite: null, decalage: null, receveurOuvert: null }).repli.role}`,
    marchant.n === 0 && presseur.n >= 1 && matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null, orientationPasse: null, verticalite: null, decalage: null, receveurOuvert: null }).repli.role === true);   // « devant » informatif DATÉ 249b : la craie tenue garde l'ailier marchant à sa ligne, pas devant le ballon (44 c. 45 %) — le mécanisme est le sprint, 0 c. 199
}
