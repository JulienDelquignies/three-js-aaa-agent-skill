...
// ---------- 1. le contrat du mapping (bandes, monotonie, no-op du 50, clés inconnues)
{
  ...
  // ROADMAP : les boucles de POSSESSION restent géométriques — passes volées 7 = 7.)
  {
    ...
    const diffDe = (n) => {
      ...
      for (const seed of [2, 3, 5, 8, 11, 13]) {
        ...
        const cfg = matchCfg({ shotRange: 20, tacleDegage: false, courseServie: false });
  ...
  // tué : le premier-éligible + la cadence personnelle font DÉJÀ vivre la note, note 197).
  {
    ...
    const jumeau = (otb) => {
      ...
      for (const seed of [2, 3, 5, 8, 9, 11]) {
        ...
        const cfg = matchCfg({ shotRange: 20, tacleVif: false, mord: false, pressZone: false, rondSort: false, compression: false, tacleDegage: false, courseServie: false });   // la clause isole 157-160 (les re-dateurs de possessions)
  ...
  // porte de discipline du 95 juge (balPrenable), et l'horloge est À LA NOTE tackling.
  {
    ...
    const vivant = armes({ mord: false, pressZone: false, rondSort: false, compression: false, tacleDegage: false, courseServie: false }), mort = armes({ mord: false, pressZone: false, rondSort: false, compression: false, tacleDegage: false, courseServie: false, tacleVif: false });   // …épinglé 159/160
    const bons = armes({ mord: false, pressZone: false, rondSort: false, compression: false, tacleDegage: false, courseServie: false }, 90), durs = armes({ mord: false, pressZone: false, rondSort: false, compression: false, tacleDegage: false, courseServie: false }, 10);
  ...
  // presseur est élu DANS SA ZONE (pénalité d'éloignement × teamF, la note teamwork nouvelle).
  {
    ...
    const jumeau = (tw) => {
      ...
      for (const seed of [2, 3, 5, 8, 9, 11]) {
        ...
        const st = makeMatch({ full: true, seed, squads: sq });
        const cfg = matchCfg({ shotRange: 20, rondSort: false, compression: false, tacleDegage: false, courseServie: false });   // la clause isole 160b (le rond re-date les possessions)
    ...
    const vivant = trav(), brut = trav({ pressZone: false, rondSort: false, compression: false, tacleDegage: false, courseServie: false });
  ...
  // La bande pressante monte avec le bloc (step × (1+fond) × workF), le piège Loi 11 couvre.
  {
    ...
    const prof = (over = {}) => {
      ...
      for (const seed of [2, 5, 9]) {
        ...
        const cfg = matchCfg({ shotRange: 20, tacleDegage: false, courseServie: false, ...over });
        for (let i = 0; i < 300 * 60; i++) {
          matchStep(st, 1 / 60, cfg);
    ...
    const poing = prof(), elastique = prof({ compression: false, tacleDegage: false, courseServie: false });
