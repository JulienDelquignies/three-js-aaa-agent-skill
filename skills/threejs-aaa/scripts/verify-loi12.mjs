...
const CFG = () => matchCfg({ ceremonie: null,  shotRange: 20, loi12: { ...L12 } });
...
};
{
  ...
  for (let i = 0; i < 6.5 * 60; i++) matchStep(st2, 1 / 60, matchCfg({ ceremonie: null,  shotRange: 20, loi12: false }));
...
// ---------- 4. sabotages nommés : la porte est la SEULE porte
{
  ...
  for (let i = 0; i < 60; i++) matchStep(st, 1 / 60, matchCfg({ ceremonie: null,  shotRange: 20, loi12: false }));
...
}
{
  ...
  adjugeFaute(st, matchCfg({ ceremonie: null,  shotRange: 20, loi12: { ...L12, avantage: 0 } }));
...
// ---------- 5. le FLUX existe et la FEUILLE compte (détection réelle, BALAYAGE)
{
  ...
  const cfg0 = matchCfg({ ceremonie: null,  shotRange: 20, loi12: false });
