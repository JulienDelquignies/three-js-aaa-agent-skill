...
for (const seed of seeds) {
  const { _tactics, ...overC } = over; for (const k of ['xg', 'qualiteTir', 'finition', 'ellipse', 'repertoire', 'enveloppe']) if (overC[k] && matchCfg({})[k]) overC[k] = { ...matchCfg({})[k], ...overC[k] };
  ...
  for (let i = 0; i < DUR * 60 * 2.4; i++) {
    ...
    for (; seen < st.events.length; seen++) { const e = st.events[seen]; const by = st.players[e.by];
      if (e.type === 'shot' && by) {
        ...
        { const path = predictPath({ p: st.ball.p, v: st.ball.v, w: st.ball.w ?? [0, 0, 0] }, { maxT: 4 }); let cr = null; for (let k = 1; k < path.length; k++) { const a = path[k - 1].p[0] - g.x, b = path[k].p[0] - g.x; if (a * b <= 0 && a !== b) { const u = a / (a - b); cr = [0, 1, 2].map((j) => path[k - 1].p[j] + (path[k].p[j] - path[k - 1].p[j]) * u); cr[3] = path[k - 1].t + (path[k].t - path[k - 1].t) * u; break; } }
          if (e.speed != null) { const K = (VK[e.kind ?? '?'] ??= { n: 0, s: 0 }); K.n++; K.s += e.speed; }
          if (cr && cr[3] != null) TF.push(cr[3]);
...
console.log(`vitesses de frappe (m/s, book : placé 20, instep 28, enroulé 24, pointu 16, volée 26, tête 13) : ${Object.entries(VK).sort((a, b) => b[1].n - a[1].n).map(([k, v]) => `${k} ${(v.s / v.n).toFixed(1)} (${v.n})`).join(', ')} ; temps de vol au plan p50 ${q(TF, 0.5).toFixed(2)} s`);
