...
let n = 0, tot = 0, totOk = 0, bloq = 0, tirs = 0, buts = 0;
...
for (const seed of seeds) {
  ...
  tirs += st.events.filter((e) => e.type === 'shot').length; buts += (st.score?.[0] ?? 0) + (st.score?.[1] ?? 0);
}
const pc = (a, b) => b ? (100 * a / b).toFixed(0) + ' %' : '—', q = (a, x) => { a = [...a].sort((u, v) => u - v); return a.length ? a[Math.floor(x * a.length)].toFixed(1) : '—'; };
console.log(`${n} × ${DUR / 60} min ${JSON.stringify(over)} — ${(tot / n).toFixed(0)} passes suivies / match, réussie (première touche d'un coéquipier) ${pc(totOk, tot)} (réel 80-83) ; bloquées < 0,4 s ${pc(bloq, tot)} (réel 3,1) ; tirs ${(tirs / n).toFixed(1)}, buts ${(buts / n).toFixed(1)} par match`);
