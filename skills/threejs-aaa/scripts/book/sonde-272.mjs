...
const o = { n: 0, shots: 0, buts: 0, arrets: 0, hors: 0, contres: 0, montants: 0, dist: [], xg: [], boite: 0, cadreIn: 0, arrIn: 0, cadreOut: 0, arrOut: 0, sdb: 0, corner: 0, tetes: 0, tetesBut: 0, kinds: {}, deny: {}, parGraine: [], bands: {}, press: 0 };
...
for (const seed of seeds) {
  const { _tactics, ...overC } = over; for (const k of ['xg', 'qualiteTir', 'finition']) if (overC[k] && matchCfg({})[k]) overC[k] = { ...matchCfg({})[k], ...overC[k] };
  ...
  for (let i = 0; i < DUR * 60 * 2.4; i++) {
    ...
    for (; seen < st.events.length; seen++) { const e = st.events[seen]; const by = st.players[e.by];
      if (e.type === 'shot' && by) {
        ...
        o.kinds[e.kind ?? '?'] = (o.kinds[e.kind ?? '?'] ?? 0) + 1; if (e.kind === 'tête') o.tetes++;
