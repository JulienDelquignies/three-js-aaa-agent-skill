...
const hyp = Math.hypot; const o = { n: 0, touches: 0, entrees: 0, shots: 0, shotsIn: 0, shotsOut: 0, poss: 0, possBox: 0, rebond: 0, cpa: 0, arb: { tir: 0, autre: 0, tirBox: 0, autreBox: 0 }, contres: 0, xg: 0, tetes: 0, centres: 0, centresIn: 0, dist: [], possLen: [] };
for (const seed of seeds) {
  const { _tactics, ...overC } = over; for (const k of ['xg', 'ligneAccrochee']) if (overC[k] && matchCfg({})[k]) overC[k] = { ...matchCfg({})[k], ...overC[k], ...(overC[k].theta && matchCfg({})[k].theta ? { theta: { ...matchCfg({})[k].theta, ...overC[k].theta } } : {}), ...(overC[k].marge && matchCfg({})[k].marge ? { marge: { ...matchCfg({})[k].marge, ...overC[k].marge } } : {}) };
  ...
  for (let i = 0; i < DUR * 60 * 2.4; i++) {
    ...
    for (; seen < st.events.length; seen++) { const e = st.events[seen];
      ...
      if (e.type === 'arbitre') { const by = st.players[e.by]; const g = st.pitch.attackGoal(by.team); const inB = st.pitch.inBox(by.p[0], by.p[2], Math.sign(g.x || 1)); if (e.choix === 'tir') { o.arb.tir++; if (inB) o.arb.tirBox++; } else { o.arb.autre++; if (inB) o.arb.autreBox++; } }
      ...
      if (e.type === 'shot') { const by = st.players[e.by]; if (!by) continue; const g = st.pitch.attackGoal(by.team), s = Math.sign(g.x || 1); o.shots++; const d = hyp(g.x - by.p[0], by.p[2]); o.dist.push(d);
        if (st.pitch.inBox(by.p[0], by.p[2], s)) o.shotsIn++; else o.shotsOut++; if (e.t - lastStop < 5) o.rebond++; if (e.kind === 'coup-franc-direct' || (st.restart && st.t - (st.restart.at ?? -99) < 3)) o.cpa++; if (e.kind === 'tête') o.tetes++; if (e.xg != null) o.xg += e.xg; }
    }
  }
}
