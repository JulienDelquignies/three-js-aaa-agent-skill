...
const cfgD = () => matchCfg({ ceremonie: null,  shotRange: 20 });
...
}
{
  ...
  const a = arbitre(st, c, matchCfg({ ceremonie: null,  shotRange: 20, menace: { tir: 1, centre: 0, passe: 0, conduite: 0 } }));
...
// ---------- 3. LE CONTRAT D'INJECTION : cfg.decide remplace la politique, le moteur obéit
{
  const scene = (over) => {
    ...
    const cfg = matchCfg({ ceremonie: null,  shotRange: 20, ...over });
