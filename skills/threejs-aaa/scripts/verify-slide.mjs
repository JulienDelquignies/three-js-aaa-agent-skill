...
const chase = (seed, arrange) => {
  ...
  const cfg = matchCfg({ flux: null /* flux null DATÉ 264 : vert à HEAD~ (au 263), la fixture force ses tirages par st.rnd (les jambes avant le ballon : faute undefined) — la clause mesure le tacle, pas le flux */, locomoteur: null /* locomoteur null DATÉ 260 : vert à HEAD~ (au 254), le dernier recours remangé par le profil locomoteur (les corps démarrent en 2,3 τ) — la clause mesure le glissé, pas la locomotion */, shotRange: 20, carton: null /* carton null DATÉ 257 : la clause « le jaune vient sur une seule glissée par derrière » mesure la récidive ×2 du 33 ; le monde 257 juge la nature (le glissé par derrière y vaut jaune à 0,91 au flux) */ });
...
// ---------- 6. le FLUX : la texture en bande, le jeu respire
{
  ...
  for (const seed of [1, 2, 3, 4, 5, 7, 9, 11, 13, 15, 17, 19]) {   // 6 → 12 graines DATÉ 240 (7 ratés secs / 6 : Poisson à un chiffre)
    const st = makeMatch({ full: true, seed });
    const cfg = matchCfg({ flux: null /* flux null DATÉ 264 : vert à HEAD~ (au 263), la fixture force ses tirages par st.rnd (les jambes avant le ballon : faute undefined) — la clause mesure le tacle, pas le flux */, locomoteur: null /* locomoteur null DATÉ 260 : vert à HEAD~ (au 254), le dernier recours remangé par le profil locomoteur (les corps démarrent en 2,3 τ) — la clause mesure le glissé, pas la locomotion */, shotRange: 20 });
...
// ---------- 7. sabotage nommé « personne ne se couche » : slideTackle:false → le monde d'hier
{
  ...
  const cfg = matchCfg({ flux: null /* flux null DATÉ 264 : vert à HEAD~ (au 263), la fixture force ses tirages par st.rnd (les jambes avant le ballon : faute undefined) — la clause mesure le tacle, pas le flux */, locomoteur: null /* locomoteur null DATÉ 260 : vert à HEAD~ (au 254), le dernier recours remangé par le profil locomoteur (les corps démarrent en 2,3 τ) — la clause mesure le glissé, pas la locomotion */, shotRange: 20, slideTackle: false });
