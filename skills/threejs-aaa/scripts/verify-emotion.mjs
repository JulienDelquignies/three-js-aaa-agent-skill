...
// ---- 5. LA SIM (cfg.fete) : le geste vient de la persona ; la glissade se lance après l'élan ; la clé absente rend l'hier
{
  const fete = (over, persona) => {
    ...
    const st = makeMatch({ full: true, seed: 3, squads }), cfg = matchCfg({ ceremonie: null, ramasseurs: null, ...over });   // (A11 ter) la cérémonie d'avant-match est d'aujourd'hui : ces clauses mesurent le monde de leur jour
