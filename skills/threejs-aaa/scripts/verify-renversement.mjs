...
// ---------- 4. sabotage nommé « jeu axial » : renversement:false → le vocabulaire d'hier
{
  ...
  const cfg0 = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null,  shotRange: 20, renversement: false });
...
// ---------- 5. le FLUX : l'orientation du jeu a changé, le jeu respire
{
  ...
  // (le patron : les bornes absolues morphent, les effets nets restent).
  {
    ...
    const ax99 = (over) => {
      ...
      for (const seed of [1, 3, 5, 7]) {
        const st = makeMatch({ full: true, seed });
        const cfg = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null,  shotRange: 20, ecarte: false, conduiteCouloir: false, ...over });
