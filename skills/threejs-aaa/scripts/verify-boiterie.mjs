...
const monde = (over) => { const st = makeMatch({ full: true, seed: 3 }); const cfg = matchCfg({ ceremonie: null, ramasseurs: null, petitsGestes: null, passements: null, enchainement: null, orientationPasse: null, verticalite: null, decalage: null, receveurOuvert: null, ...over }); for (let i = 0; i < 60; i++) matchStep(st, 1 / 60, cfg); return { st, cfg }; };
...
{
  const { st, cfg } = monde({ boiterie: null, entrant: null, petitsGestes: null, passements: null, enchainement: null, orientationPasse: null, verticalite: null, decalage: null, receveurOuvert: null });
  const V = st.players.find((q) => q.team === 0 && !q.keeper), D = st.players.find((q) => q.team === 1 && !q.keeper);
  st._faute = { t: st.t, par: D.id, sur: V.id, team: V.team, p: [V.p[0], V.p[2]], grave: true, kind: 'tacle-glissé-derrière', vSur: 3, dir: [1, 0] };
  const n0 = st.events.length; for (let i = 0; i < 60 * 3; i++) matchStep(st, 1 / 60, cfg);
