...
const W = jouer(true, [1, 2, 3, 4, 5, 6, 7, 8], 120, { conduite: false }), tenu = (R) => R.tenu / Math.max(1, R.conduite), derr = (R) => R.derriere / Math.max(1, R.libre);
ok(tenu(A) <= 0.08 && tenu(W) > 0.15, `en course le ballon n'est plus TENU au servo (une force sans pied) : ${(100 * tenu(A)).toFixed(0)} % du temps de conduite (plafond 8 % — la tenue dos au presseur, le ballon quasi mort) ; sabotage conduite:false (le servo d'hier) : ${(100 * tenu(W)).toFixed(0)} %, la clause mord`);
ok(derr(A) <= 0.18 && derr(W) > derr(A), `le porteur ne dépasse pas son ballon : ballon libre DERRIÈRE lui ${(100 * derr(A)).toFixed(0)} % du temps (plafond 18 %) ; sabotage conduite:false : ${(100 * derr(W)).toFixed(0)} %, la clause mord`);
