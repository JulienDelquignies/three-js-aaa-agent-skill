...
// ---------- 3. les axes de flux : hauteur et largeur BOUGENT leurs instruments
{
  const run = (t0, seed) => {
    ...
    const cfg = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null,  shotRange: 20, contre: null, effort: null /* effort null DATÉ 261 : vert à HEAD~ (worktree edda355), l'axe hauteurBloc rend 4,3 c. 4,5 m de médiane (la ligne suit son slot en marchant : le bloc qui monte, monte moins vite — un prix nommé au 339) — la clause mesure l'axe, pas l'intention d'effort */ })   /* contre null DATÉ 258b : vert à HEAD~ (médiane > 4,5 au 258), la ligne remangée par le corps qui contre (4,49 arrondi 4,5) — la clause mesure la hauteur du bloc, pas le contre */;
  ...
  const cibleAiliers = (t0) => {
    ...
    const cfg = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null,  shotRange: 20, renversement: false });
...
// ---------- 5. le STYLE départage les choix SERRÉS (l'arbitre par équipe)
{
  const monde = (style) => {
    ...
    st.restart = null; st.ball.possess(c.id);
    st.possession = { team: 0, carrier: c.id }; st.phase = 'carry'; st.hold = 1.0; st.lastTouch = 0;
    return arbitre(st, c, matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null,  xg: null /* xg null DATÉ 272 : vert à HEAD~ (11/0 au 271), le choix serré remangé (direct → passe : la porte xG compare le tir à sa continuation) — la clause mesure sa loi, pas le xG */, shotRange: 20 }));
  };
...
// de la porte — aucun tirage consommé), direct 0 calme. Trois mondes, la même graine.
{
  const utDe = (tactics, cfgX) => {
    ...
    for (const seed of [1, 3, 5, 7, 9, 11]) {   // 3 → 6 graines DATÉ 241 (1 calme sur 1 une-touche : Poisson)
      ...
      const cfg = matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null,  shotRange: 20, uneToucheVive: false, uneTouche: { ...matchCfg({ ceremonie: null, ramasseurs: null, boiterie: null, entrant: null, petitsGestes: null, passements: null }).uneTouche, base: 0 }, ...cfgX });
