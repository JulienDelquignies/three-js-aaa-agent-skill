...
export function relancerGardien(st, gk, cfg, deps) {
  ...
  if (styleSB === 'court') {
    ...
    if (libre) {
      ...
      if (deps.beginPass(st, { to: { id: libre.id }, lead, style: 'ground', lane: { margin: 6 } }, cfg, { forceUrgent: true, mains: !!(st.full && cfg.remisesMain && cfg.remisesMain.roule !== false) })) {   // (lot A9, cfg.remisesMain) le roulé — absente ou roule:false : la passe du pied d'hier
