  ...
  for (const p of attackers) {
    if (carrier && p.id === carrier.id) {
      ...
      if (cfg.carryViaBall !== false && (dBall > 0.85 || (st.full && cfg.conduite?.couple && dBall > 0.25 && p.push && (st.ball.p[0] - p.p[0]) * p.push[0] + (st.ball.p[2] - p.p[2]) * p.push[1] < 0.2 * dBall))) {   // (cfg.conduite) le ballon PAS DEVANT (derrière, de côté) : on va le chercher, même à portée — viser 3 m devant laissait le ballon derrière (12 % du temps libre)
        ...
        const over = (p._prepShot ?? -1) > st.t && !(st.full && cfg.conduite?.couple && p.push && ((st.ball.p[0] - p.p[0]) * p.push[0] + (st.ball.p[2] - p.p[2]) * p.push[1]) < 0.9 * dBall) ? 2.2 : 0.4;   // (cfg.conduite) abordé de biais (> 25°), on va D'ABORD au ballon : viser 2,2 m au-delà passait à 1 m du ballon mort, sans le toucher
