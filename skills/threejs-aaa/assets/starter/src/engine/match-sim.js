...
import { BALL } from './ball.js'; import { laneClearance, predictPath, interceptPoint, etaCourse } from './ball-predict.js'; import { cibleFoulee, cibleAlignee } from './foulee.js'; import { elanConduite } from './elan-conduite.js'; import { suiviAppelStep } from './suivi-appel.js'; import { repliPasseStep } from './repli-passe.js'; import { suiviPasseurStep } from './suivi-passeur.js'; import { repliStep } from './repli.js'; import { lossReactStep, contrePressStep } from './contrepress.js'; import { compenserLateral } from './compensation.js'; import { projeterMilieux, postesEntreLignes } from './projection.js'; import { couvertStep } from './couvert.js'; import { gardeDist } from './garde.js'; import { salidaStep, conduccion } from './salida.js'; import { cfSpots, remiseCible, sortieBalle } from './cpa.js'; import { affecterMarquage, refermerLigne , bandeDuCentral, remettreAuPivot, hommeRemis, purgerPassation } from './marquage.js'; import { RONDO, makeRondo, evadeSpot, gapZ } from './rondo.js';
...
function assignMatchJobs(st, cfg) {
  ...
  for (const p of attackers) {
    if (carrier && p.id === carrier.id) {
      ...
      if (st.full && cfg.elanConduite) [px, pz] = elanConduite(st, p, cfg.elanConduite, px, pz, sgnG);   /* (312) l'élan dans l'espace — elan-conduite.js */ const pl = hyp(px, pz) || 1;
  ...
  if (flightRec && !flightRec.keeper && flightRec.team === atk) {
    ...
    const finVol = st.full && cfg.appuisRecev && dInb / Math.max(1, hyp(st.ball.v[0], st.ball.v[2])) < (cfg.appuisRecev.fen ?? 0.8); if (st.full && cfg.sePoser && st.ball.p[1] < 0.9) { const vb = hyp(st.ball.v[0], st.ball.v[2]), vr = hyp(flightRec.v[0], flightRec.v[1]); if (vb > 1 && vr > 0.5 && dInb / vb < (cfg.sePoser.fen ?? 0.5) && (flightRec.v[0] * st.ball.v[0] + flightRec.v[1] * st.ball.v[2]) / (vb * vr) < (cfg.sePoser.cos ?? -0.3)) flightRec._pose = st.t + 0.1; }   /* (303) LE RECEVEUR FACE AU BALLON SE POSE (movement : la pointe capée à sePoser.v) — retour du 25/09 « un énorme problème d'appui » : 43 des 51 touches lourdes venaient d'un receveur COURANT dans le ballon (4,3 m/s, v relative 10) ; absent : hier au bit */
    ...
    if (!fouleeLock && st.full && cfg.aligneRecev) { const A = cibleAlignee(st, cfg, flightRec, pitch); if (A) met = A; }   /* (309) le receveur s'aligne sur le vol avant la prise — doc foulee.js */ if (met) { met[0] = Math.max(-pitch.hx + 0.8, Math.min(pitch.hx - 0.8, met[0])); met[2] = Math.max(-pitch.hz + 0.8, Math.min(pitch.hz - 0.8, met[2])); }
  ...
  boxCrashStep(st, cfg, { busy, tac, axe, role, d2 }); contreZonesStep(st, cfg, { tac, axe, role }); suiviAppelStep(st, cfg, { tac, axe, busy }); repliPasseStep(st, cfg, { tac, axe, busy }); suiviPasseurStep(st, cfg, { busy });   /* (317) l'appel en profondeur est pris — suivi-appel.js */   // 242 : les trois zones d'entrée en contre (phases.js) ; 123/182b : la géométrie du centre remplit la boîte ET les corps ATTAQUENT le vol (phases.js)
