...
export function buildCity(city, { at = [0, 0, 0] } = {}) {
  ...
  // buildings: ONE InstancedMesh, per-instance scale + color (flat-shaded blocks read great from above)
  if (city.buildings.length) {
    const bg = new THREE.BoxGeometry(1, 1, 1); disposables.push(bg);
    const bm = new THREE.InstancedMesh(bg, mat({ roughness: 0.85 }), city.buildings.length);
    bm.instanceColor = new THREE.InstancedBufferAttribute(new Float32Array(city.buildings.length * 3), 3);
