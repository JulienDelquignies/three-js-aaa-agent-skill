...
- **Overcoming the "no art / browser budget" limits** — optionally orchestrating paid AI generation
  ...
  THE MATCH READ-THROUGH OF 17/09 (`cfg.orientationPasse`, `cfg.verticalite`, `cfg.decalage`, `cfg.receveurOuvert`): the pass technique is chosen for the turn it has to make and the free carrier opens up before winding up (0 planned passes leaving more than 60° off the facing, was 10 of 42; the kick-off leaves at 27° in 1.1 s); the forward pass gets its sweet spot back and space ahead gets its own range while the long back pass pays; the running carrier attacks the defender's shoulder and the hook and the stepover exist on the run (hooks 0 → 3-4 per 10 min); the receiver running away from the ball opens up during the flight (runners served in the back 60 → 29-37 %). Every key null reproduces d3dbe73 bit for bit.
