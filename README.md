...
- **Overcoming the "no art / browser budget" limits** — optionally orchestrating paid AI generation
  ...
  ITS SECOND PASS (`cfg.toucheOrientee`, `cfg.verticalite.appel`, `cfg.orientationPasse.talon`, `cfg.decalage.sortie`): the free receiver's first touch takes the ball into the open side instead of capturing it (welded carry frames 49 → 38 %), the free man in space is served at the base hold (losses 76 → 67 on four seeds), the heel pass is honest (the body faces away from the target), the bitten defender sits at least 0.6 s after a sold feint.
