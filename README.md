...
│   ├── 58-signes-visibles-du-role.md  THE VISIBLE SIGNS OF THE ROLE (lot A12): the receiver SCANS during the pass flight — the gaze layer reads the sim's scan clock (p.scan) instead of pinning the eyes on the ball, eyes back on the ball for the take; the receiver's POSTURE (ready stance, arms in balance in the slow stride), the PAUSA (sole on the real ball, hands on hips), the JOCKEY's low open stride, the walking roles' hands on hips, the corner taker's raised arm, the no-look pass; then THE DEBTS TAKEN BACK as keyed sim laws (the pausa's sole stop, the receive-in-place, the cross receiver's step-back), each null = yesterday bit for bit
