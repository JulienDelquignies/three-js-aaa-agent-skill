...
- **Overcoming the "no art / browser budget" limits** — optionally orchestrating paid AI generation
  ...
  THE GESTURE INVENTORY (`docs/Inventaire_Gestes.md`, generated from the generator registry): 75 generated gestures, all wired to a sim trigger or a scene choice once the last orphan, `applaudir`, plays on a keeper's save (up to two free teammates within 18 m, one wave per team per 8 s) and at the final salute (every other player applauds the crowd). Named debt: the carrier's dribble touches are `control` events without a technique, so the scene plays the inside control by default.
